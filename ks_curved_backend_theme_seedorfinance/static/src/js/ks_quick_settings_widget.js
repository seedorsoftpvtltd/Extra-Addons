odoo.define(
  "ks_curved_backend_theme_seedorfinance.ks_quick_settings_widget",
  function (require) {
    "use strict";
    ...
    var KsQuickSettings = Widget.extend({
      ...
      willStart: async function () {
        ...
        const ks_current_theme = await this._rpc({
          route: "/ks_curved_backend_theme_seedorfinance/getTheme",
          params: {},
        });
        ...
        session.ks_curved_backend_theme_seedorfinance_data = data;
        ...
        if (ks_dynamic_css && ks_dynamic_css.ks_button_style) {
          ajax.loadCSS(
            `/ks_curved_backend_theme_seedorfinance/static/src/scss/components/buttons/${ks_dynamic_css.ks_button_style}.css`
          );
        }
        if (ks_dynamic_css && ks_dynamic_css.ks_theme_style) {
          ajax.loadCSS(
            `/ks_curved_backend_theme_seedorfinance/static/src/scss/components/theme_style/${ks_dynamic_css.ks_theme_style}.css`
          );
        }
        if (ks_dynamic_css && ks_dynamic_css.ks_tab_style) {
          ajax.loadCSS(
            `/ks_curved_backend_theme_seedorfinance/static/src/scss/components/tab_style/${ks_dynamic_css.ks_tab_style}.css`
          );
        }

        if (ks_dynamic_css && ks_dynamic_css.ks_font_style) {
          ajax.loadCSS(
            `/ks_curved_backend_theme_seedorfinance/static/src/scss/components/fonts/${ks_dynamic_css.ks_font_style}.css`
          );
        }

        if (ks_dynamic_css && ks_dynamic_css.ks_separator_style) {
          ajax.loadCSS(
            `/ks_curved_backend_theme_seedorfinance/static/src/scss/components/separators/${ks_dynamic_css.ks_separator_style}.css`
          );
        }

        if (ks_dynamic_css && ks_dynamic_css.ks_checkbox_style) {
          ajax.loadCSS(
            `/ks_curved_backend_theme_seedorfinance/static/src/scss/components/checkbox/${ks_dynamic_css.ks_checkbox_style}.css`
          );
        }

        if (ks_dynamic_css && ks_dynamic_css.ks_font_size) {
          ajax.loadCSS(
            `/ks_curved_backend_theme_seedorfinance/static/src/scss/components/fontsize/${ks_dynamic_css.ks_font_size}.css`
          );
        }

        if (ks_dynamic_css && ks_dynamic_css.ks_chatter == "ks_chatter_right") {
          ajax.loadCSS(
            `/ks_curved_backend_theme_seedorfinance/static/src/scss/components/chatter/${ks_dynamic_css.ks_chatter}.css`
          );
        }

        if (ks_dynamic_css && ks_dynamic_css.ks_radio_button_style) {
          ajax.loadCSS(
            `/ks_curved_backend_theme_seedorfinance/static/src/scss/components/radiobutton/${ks_dynamic_css.ks_radio_button_style}.css`
          );
        }

        if (ks_dynamic_css && ks_dynamic_css.ks_popup_animation_style) {
          ajax.loadCSS(
            `/ks_curved_backend_theme_seedorfinance/static/src/scss/components/popup_animation/${ks_dynamic_css.ks_popup_animation_style}.css`
          );
        }
      ...
      _onInputChange: function (ev) {
        ...
        if (ev.target.name) {
          ...
          if (
            ...
            _value &&
            !session.ks_curved_backend_theme_seedorfinance_data.ks_sun_time_info
          ) {
      ...
      _ksGlobalSettings: function (ev) {
        ...
        var dict = {
          ...
          context: {
            form_view_ref:
              "ks_curved_backend_theme_seedorfinance.ks_global_configuration_form",
          },
        };
      ...
      _ksResetValues: function () {
        for (var index in this.ks_unsaved_setting) {
          if (typeof session.ks_curved_backend_theme_seedorfinance_data[index] == "boolean") {
            $("input#" + index).prop(
              "checked",
              session.ks_curved_backend_theme_seedorfinance_data[index]
            );
            if (index == "ks_dark_mode") {
              if (session.ks_curved_backend_theme_seedorfinance_data[index])
            ...
            $("input#" + session.ks_curved_backend_theme_seedorfinance_data[index]).prop(
              "checked",
              true
            );
            delete this.ks_unsaved_setting[index];
        ...
        if ("ks_dark_mode" in this.ks_unsaved_setting) {
          this._ksManageAutoDarkMode(
            session.ks_curved_backend_theme_seedorfinance_data.ks_dark_mode
          );
        }
        // Hide/Show Auto-dark mode
        if ("ks_dark_mode" in this.ks_unsaved_setting) {
