    ...
    def build_detailed_move_lines(self, offset=0, partner=0, fetch_range=FETCH_RANGE):
        ...
        sql = ('''
                        SELECT
                            ...
                            l.date AS ldate,
                            j.code AS lcode,
                            m.supplier_invoice_number AS msupplier_invoice_number,
                            l.currency_id,
                            l.amount_currency,
                            --l.ref AS lref,
                            CASE WHEN j.type = 'sale' OR j.type = 'purchase' THEN am.narration ELSE l.name END AS lname,
                            m.id AS move_id,
                            m.name AS move_name,
                        ...
                        WHERE %s
                        GROUP BY l.id, l.partner_id, a.name, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.name, m.id, m.name, c.rounding, cc.id, cc.rounding, cc.position, c.position, c.symbol, cc.symbol, p.name, j.type, am.narration
                        ORDER BY %s
                        OFFSET %s ROWS
                        FETCH FIRST %s ROWS ONLY
