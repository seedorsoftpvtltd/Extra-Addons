...
class ReportWizard(models.TransientModel):
    ...
    def view_stock_movement(self):
        if self.product_id:
            ...
            # changes made 05/10/23 by ARUN
            # From
            if self.partner_id.id and self.start_date and self.end_date is not None:
                domain = [('owner_id', '=', self.partner_id.id), ('create_date', '>=', self.start_date),
                          ('create_date', '<=', self.end_date), ('picking_code', 'in', ['incoming', 'outgoing']),
                          ('product_id.item', '!=', False),('state','=','done')
                          ]
            elif self.start_date and self.end_date and not self.partner_id.id:
