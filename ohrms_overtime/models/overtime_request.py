...
class HrOverTime(models.Model):
    ...
    attendance_ids = fields.Many2many('hr.attendance', store=True, string='Attendance')
        ...
        if self.contract_id and self.date_from and self.date_to:
            ...
            for rec in self:
                ...
                if rec.employee_id.contract_id:
                    ...
                    # time = rec.date_to.strftime("%H:%M:%S")
                    time = datetime.strptime(str(rec.date_to), '%Y-%m-%d %H:%M:%S')
                    t = time + timedelta(hours=5, minutes=30)
                    t = t.strftime("%H:%M:%S")

                    work_schedule = rec.sudo().employee_id.contract_id.resource_calendar_id
                    for schedule in work_schedule.sudo().attendance_ids:

                        # if schedule.day_period == 'afternoon':

                        # work_to = schedule.hour_to

                        d = datetime.now().date()
                        #                            d = datetime.strptime(str(self.date_from), '%Y-%m-%d').date()
                        #                            d = self.date_from.date()

                        # if str(t) >= str(work_to):

                        hr_attendance = self.env['hr.attendance'].search(

                            [('employee_id', '=', self.employee_id.id)]).filtered(
                            lambda filter: filter.check_in.date() <= self.date_to.date())

                        for rec in hr_attendance.filtered(
                                lambda attendance: attendance.check_in.date() == self.date_from.date()):
                            self.update({
                                'attendance_ids': [(6, 0, rec.ids)]
...
#                            d = self.date_from.date()
                             ...
                             hr_attendance = self.env['hr.attendance'].search(
                                ...
                                [('employee_id', '=', self.employee_id.id)]).filtered(lambda filter: filter.check_in.date() <= self.date_to.date())

                             for rec in hr_attendance.filtered(lambda attendance: attendance.check_in.date() == self.date_from.date()):
