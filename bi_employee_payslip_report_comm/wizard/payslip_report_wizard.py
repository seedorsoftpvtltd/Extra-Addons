...
class EmpPayslipReport(models.TransientModel):
    ...
    def employee_payslip_xls(self):
        if self.file_type == 'pdf':
            ...
            return self.env.ref('bi_employee_payslip_report_comm.action_report_export_emp_payslip').report_action(self,
                                                                                                                  data=datas)
            ...
            for payslip in payslip_ids:
                ...
                for i, j in lable.items():
                    if i not in ['no', 'payslip_ref', 'employee', 'designation', 'period', 'supervisor', 'bank_name', 'work_days']:
            ...
            for value in main:
                col = 0
                row = row
                list = ['NO', 'Payslip_Ref', 'Employee', 'Designation', 'Period', 'Supervisor',  'Bank_Name','Work_Days', 'Basic', 'Allowance', 'Gross',
                        'Deduction', 'Net']
                for l in list:
                    if l in value.keys():
                        data = value.get(l)
