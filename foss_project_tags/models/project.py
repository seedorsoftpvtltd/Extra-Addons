...
class Project(models.Model):
    ...
    date_start = fields.Date(string='Start Date', default=fields.Datetime.now, track_visibility='onchange')
    date = fields.Date(string='End Date', index=True, track_visibility='onchange', default=fields.Datetime.now)
#    datetest = fields.Date(string='Date Test', default=time.strftime('%Y-%m-%d'))
 #   datetes = fields.Date(string='Date Tes', default=fields.Datetime.now)
...
class Task(models.Model):
    ...
    def write(self, vals):
        ...
        if vals.get('date_deadline') and self.date_start:
            deadline = datetime.strptime(str(vals.get('date_deadline')), '%Y-%m-%d').date()
         ...
         #   date_start = datetime.strptime(vals.get('date_start'), '%Y-%m-%d').date()
          #  if self.date_deadline < date_start:
           #     raise UserError(('Deadline cannot be lesser than the starting date'))
        if vals.get('date_deadline') and vals.get('date_start'):
              date_start = datetime.strptime(str(vals.get('date_start')), '%Y-%m-%d').date()
              date_deadline = datetime.strptime(str(vals.get('date_deadline')), '%Y-%m-%d').date()
...
#           date_deadline = datetime.strptime(vals.get('date_deadline'), '%Y-%m-%d').date()
            ...
            if vals.get('date_start'):
               ...
               # if project_starting_date > starting_dt:
                ...
                if project_starting_date > date_start:
    ...
    def onchange_check_date(self):
        if self.date_start and self.date_deadline:
            if self.date_start > self.date_deadline:
                raise UserError('Start Date cannot be greater than deadline')
    #@api.onchange('date_start', 'date_deadline')
    #def onchange_check_date(self):

        #date_start = datetime.strptime(str(self.date_start), '%Y-%m-%d').date()
        #date_deadline = datetime.strptime(str(self.date_deadline), '%Y-%m-%d').date()
