...
class BaseModel(models.AbstractModel):
                        ...
                        if disable == 'False':
                            ...
                            if value:
                                activity_log_obj.create({
                                    ...
                                    'model_id':self.env['ir.model'].search([('model','=',record._name)],limit=1).id,
                                    'res_id':record.id,
                                    'action':'edit',
    ...
    def create(self, vals_list):
        ...
        moduel_state = self.env['ir.module.module'].search([('name','=','advanced_session_management')]).state
        config_parameter_obj = self.env['ir.config_parameter'].sudo()
            ...
            if record.name != 'advanced_session_management':
                ...
                if login_log:
                    ...
                    record.env['activity.log'].sudo().create({
                        'name':record.display_name,
                        'model_id':record.env['ir.model'].search([('model','=',record._name)],limit=1).id,
                        'res_id':record.id,
                        'action':'edit',
                        'login_log_id':login_log.id,
                        'user_id':login_log.user_id.id,
