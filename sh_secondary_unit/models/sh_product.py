...
class sh_product_product(models.Model):
    ...
    category_id = fields.Many2one("uom.category", "Category", related="uom_id.category_id")
...
class sh_stock_quant(models.Model):
    ...
    sh_secondary_unit_qty = fields.Float('On Hand',compute='_get_secondary_qty')
    sh_secondary_unit = fields.Many2one('uom.uom', 'Secondary UOM', related='product_id.sh_secondary_uom')
    ...
    def _get_secondary_qty(self):
        for rec in self:
            ...
            if rec.quantity > 0.0 and rec.product_id.sh_secondary_uom:
                rec.sh_secondary_unit_qty = rec.product_uom_id._compute_quantity(rec.quantity, rec.product_id.sh_secondary_uom)
