...
class InsuranceDetails(models.Model):
    ...
    _sql_constraints = [
        ('card_code_uniq', 'unique(card_code)', 'Insurance Card Code must be unique!'),
    ]
    ...
    amount = fields.Float(string='Premium', help="Policy amount")
    ...
    def company_insurance(self):
        for rec in self:
            if rec.insurance_percentage:
                print(rec.insurance_percentage)
                # rec['premium_amount'] = 100
                rec.premium_amount =rec.premium_amount-((rec.premium_amount*rec.insurance_percentage)/100)
    ...
    def view_insurance(self):
        ...
        """
        self.ensure_one()
        form_view = self.env.ref('hr_medical.insurance_details_form_view')
        return {
    ...
    def view_claims(self):
        ...
        self.ensure_one()
        if self.claims_ids:
            tree_view = self.env.ref('hr_medical.claims_details_tree_view')
            form_view = self.env.ref('hr_medical.claim_details_form_view')
            return {
                'type': 'ir.actions.act_window',
                'name': _('Claims'),
    ...
    def check_insurance_expiry(self):
        ...
        """
        try:
            template_id = self.env.ref('hr_medical.hr_medical_insurance_expiration_email')
        except ValueError:
            template_id = False
        for insurance in self.search(['|',('state', '=', 'done'),('state','=','confirmed')]):
...
class InsurancePremium(models.Model):
    ...
    def create_invoice(self):
        ...
        """
        product_id = self.env.ref('hr_medical.insurance_prodcuct')
    ...
    def action_invoice_create(self):
        """
            Create Invoice for Premium Amount
        """
        premiums = self.search([('insurance_id.state', 'in',('done','confirmed')), ('is_invoice_created', '=', False), ('date', '=', fields.date.today())])
        for premium in premiums:
            premium.create_invoice()

    def unlink(self):
        """
            To remove the record, which is not in 'confirm', 'refuse' and 'done' states
        """
        for line in self:
            if line.is_invoice_created:
                raise UserError(_('You cannot remove the record for which an invoice is created'))
        return super(InsurancePremium, self).unlink()
...
class ClaimDetails(models.Model):
    ...
    def unlink(self):
        ...
        """
        for line in self:
            if line.state in ['confirm', 'refuse', 'done']:
                raise UserError(_('You cannot remove the record which is in %s state!') % (line.state))
        return super(ClaimDetails, self).unlink()
...
class HrInsurance(models.Model):
    ...
    insurance = fields.One2many('insurance.details', 'employee_id', string="Insurance", help="Insurance",
                                domain=['|',('state', '=', 'done'),('state', '=', 'confirmed')])
    ...
    def get_deduced_amount(self):
        ...
        for emp in self:
            ...
            for ins in emp.insurance:
                x = str(ins.start_date)
                y = str(ins.end_date)
                if x < current_datetime:
                    if y > current_datetime:
                        if ins.premium_type == 'monthly':
                            ins_amount = ins_amount + (ins.premium_amount*12)

                        else:
                            ins_amount = ins_amount + ins.premium_amount
