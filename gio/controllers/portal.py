...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self):
        ...
        SaleOrder = request.env['goods.issue.order']
    ...
    def portal_my_quotes(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        archive_groups = self._get_archive_groups('goods.issue.order', domain) if values.get('my_details') else []
        ...
        return request.render("gio.portal_my_quotations", values)
    ...
    def portal_my_orders(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        return request.render("gio.portal_my_orders", values)

    @http.route(['/my/orders/<int:order_id>'], type='http', auth="public", website=True)
    def portal_order_page(self, order_id, report_type=None, access_token=None, message=False, download=False, **kw):
        try:
            order_sudo = self._document_check_access('goods.issue.order', order_id, access_token=access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')

        if report_type in ('html', 'pdf', 'text'):
            return self._show_report(model=order_sudo, report_type=report_type, report_ref='gio.action_report_goodsorder', download=download)
        ...
        return request.render('gio.sale_order_portal_template', values)
    ...
    def portal_quote_accept(self, order_id, access_token=None, name=None, signature=None):
        ...
        pdf = request.env.ref('gio.action_report_goodsorder').sudo().render_qweb_pdf([order_sudo.id])[0]

        _message_post_helper(
            'goods.issue.order', order_sudo.id, _('Order signed by %s') % (name,),
    ...
    def decline(self, order_id, access_token=None, **post):
        ...
        if order_sudo.has_to_be_signed() and message:
            ...
            _message_post_helper('goods.issue.order', order_id, message, **{'token': access_token} if access_token else {})
        else:
    ...
    def payment_transaction_token(self, acquirer_id, order_id, save_token=False, access_token=None, **kwargs):
        ...
        order = request.env['goods.issue.order'].sudo().browse(order_id)
