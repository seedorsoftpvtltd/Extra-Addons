...
class TestSaleOrder(TestCommonSaleNoChart):
    ...
    def setUpClass(cls):
        ...
        cls.user_in_team = cls.env['res.users'].create({
            ...
            'login': 'team0user',
            'name': 'User in Team 0',
            'sale_teams_id': cls.crm_team0.id
        })
    ...
    def test_reconciliation_with_so(self):
        ...
        self.assertFalse(line.get('goods_order_ids', False))
        ...
        line = res.get('lines', [{}])[0]
        self.assertEquals(line.get('goods_order_ids', []), [so.id])
    ...
    def test_assign_sales_team_from_partner_user(self):
        ...
        partner = self.env['res.partner'].create({
            ...
            'teams_id': self.crm_team1.id,
        })
        sale_order = self.env['sale.order'].create({
            'partner_id': partner.id,
        })
        sale_order.onchange_partner_id()
        self.assertEqual(sale_order.teams_id.id, self.crm_team0.id, 'Should assign to team of sales person')
    ...
    def test_assign_sales_team_from_partner_team(self):
        ...
        sale_order.onchange_partner_id()
        self.assertEqual(sale_order.teams_id.id, self.crm_team1.id, 'Should assign to team of partner')
    ...
    def test_keep_sales_team_when_changing_user_with_no_team(self):
        ...
        sale_order.onchange_user_id()
        self.assertEqual(sale_order.teams_id.id, self.crm_team1.id, 'Should not reset the team to default')
