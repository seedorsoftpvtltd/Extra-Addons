...
class TestAccessRights(TestCommonSaleNoChart):
    ...
    def setUp(self):
        ...
        self.order = self.env['goods.issue.order'].with_context(tracking_disable=True).create({
    ...
    def test_access_sales_manager(self):
        ...
        SaleOrder = self.env['goods.issue.order'].with_context(tracking_disable=True)
        ...
        self.assertIn(india_channel.id, self.env['crm.team'].search([]).ids, 'Manager should be able to create a GIO Team')
        ...
        self.assertNotIn(india_channel.id, self.env['crm.team'].search([]).ids, 'Manager should be able to delete a GIO Team')
    ...
    def test_access_sales_person(self):
        ...
        self.order.with_user(self.user_salesperson_1).write({'teams_id': self.sales_channel.id})
        # Salesperson can't create the SO of other salesperson
        with self.assertRaises(AccessError):
            self.env['goods.issue.order'].with_user(self.user_salesperson_1).create({
                'partner_id': self.partner_customer_usd.id,
                'user_id': self.user_salesperson.id
            })
    ...
    def test_access_portal_user(self):
        ...
        with self.assertRaises(AccessError):
            self.order.with_user(self.user_portal).write({'teams_id': self.sales_channel.id})
        # Portal user can't create the SO
        with self.assertRaises(AccessError):
            self.env['goods.issue.order'].with_user(self.user_portal).create({
                'partner_id': self.partner_customer_usd.id,
            })
    ...
    def test_access_employee(self):
        ...
        with self.assertRaises(AccessError):
            self.order.with_user(self.user_employee).write({'teams_id': self.sales_channel.id})
        # Employee can't create the SO
        with self.assertRaises(AccessError):
            self.env['goods.issue.order'].with_user(self.user_employee).create({
                'partner_id': self.partner_customer_usd.id,
            })
