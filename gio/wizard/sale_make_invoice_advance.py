...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    def _default_product_id(self):
        product_id = self.env['ir.config_parameter'].sudo().get_param('gio.default_deposit_product_id')
    ...
    def _default_has_down_payment(self):
        if self._context.get('active_model') == 'goods.issue.order' and self._context.get('active_id', False):
            sale_order = self.env['goods.issue.order'].browse(self._context.get('active_id'))
    ...
    def create_invoices(self):
        sale_orders = self.env['goods.issue.order'].browse(self._context.get('active_ids', []))
        ...
        if self.advance_payment_method == 'delivered':
            ...
            if not self.product_id:
                ...
                self.env['ir.config_parameter'].sudo().set_param('gio.default_deposit_product_id', self.product_id.id)

            sale_line_obj = self.env['goods.order.line']
