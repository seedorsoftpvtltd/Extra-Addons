...
class ResCompany(models.Model):
    ...
    def action_open_sale_onboarding_payment_acquirer(self):
        ...
        action = self.env.ref('gio.action_open_sale_onboarding_payment_acquirer_wizard').read()[0]
        return action
    ...
    def _get_sample_sales_order(self):
        ...
        sample_sales_order = self.env['goods.issue.order'].search(
            [('company_id', '=', company_id), ('partner_id', '=', partner.id),
             ('state', '=', 'draft')], limit=1)
        if len(sample_sales_order) == 0:
            sample_sales_order = self.env['goods.issue.order'].create({
            ...
            self.env['goods.order.line'].create({
    ...
    def action_open_sale_onboarding_sample_quotation(self):
        ...
        template = self.env.ref('gio.email_template_edi_sale', False)
        action = self.env.ref('gio.action_open_sale_onboarding_sample_quotation').read()[0]
        action['context'] = {
