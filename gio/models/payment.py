...
class PaymentTransaction(models.Model):
    ...
    goods_order_ids = fields.Many2many('goods.issue.order', 'goods_issue_order_transaction_rel', 'transaction_id', 'goods_order_id',
                                      string='Goods Orders', copy=False, readonly=True)
    goods_order_ids_nbr = fields.Integer(compute='_compute_sale_order_ids_nbr', string='# of Sales Orders')
    ...
    @api.depends('goods_order_ids')
    def _compute_sale_order_ids_nbr(self):
        for trans in self:
            trans.goods_order_ids_nbr = len(trans.goods_order_ids)
    ...
    def _log_payment_transaction_sent(self):
        ...
        for trans in self:
            ...
            for so in trans.goods_order_ids:
    ...
    def _set_transaction_pending(self):
        ...
        for record in self:
            sales_orders = record.goods_order_ids.filtered(lambda so: so.state in ['draft', 'sent'])
            ...
            if record.acquirer_id.provider == 'transfer':
                for so in record.goods_order_ids:
    ...
    def _check_amount_and_confirm_order(self):
        self.ensure_one()
        for order in self.goods_order_ids.filtered(lambda so: so.state in ('draft', 'sent')):
    ...
    def _set_transaction_authorized(self):
        ...
        sales_orders = self.mapped('goods_order_ids').filtered(lambda so: so.state in ('draft', 'sent'))
        for tx in self:
    ...
    def _reconcile_after_transaction_done(self):
        ...
        if self.env['ir.config_parameter'].sudo().get_param('gio.automatic_invoice'):
            default_template = self.env['ir.config_parameter'].sudo().get_param('gio.default_email_template')
            if default_template:
                for trans in self.filtered(lambda t: t.goods_order_ids):
        ...
        if self.env['ir.config_parameter'].sudo().get_param('gio.automatic_invoice'):
            for trans in self.filtered(lambda t: t.goods_order_ids):
                ctx_company = {'company_id': trans.acquirer_id.company_id.id,
                               'force_company': trans.acquirer_id.company_id.id}
                trans = trans.with_context(**ctx_company)
                trans.goods_order_ids._force_lines_to_invoice_policy_order()
                invoices = trans.goods_order_ids._create_invoices()
                trans.invoice_ids = [(6, 0, invoices.ids)]

    @api.model
    def _compute_reference_prefix(self, values):
        prefix = super(PaymentTransaction, self)._compute_reference_prefix(values)
        if not prefix and values and values.get('goods_order_ids'):
            many_list = self.resolve_2many_commands('goods_order_ids', values['goods_order_ids'], fields=['name'])
    ...
    def action_view_sales_orders(self):
        ...
        goods_order_ids = self.goods_order_ids.ids
        if len(goods_order_ids) == 1:
            action['res_id'] = goods_order_ids[0]
            action['view_mode'] = 'form'
        else:
            action['view_mode'] = 'tree,form'
            action['domain'] = [('id', 'in', goods_order_ids)]
        return action
