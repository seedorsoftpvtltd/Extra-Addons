...
class ProductTemplate(models.Model):
    ...
    service_type = fields.Selection([('manual', 'Manually set quantities on order')], string='Track Service',
        help="Manually set quantities on order: Invoice based on the manually entered quantity, without creating an analytic account.\n"
             ...
             "Create a task and track hours: Create a task on the Goods order validation and track the work hours.",
        default='manual')
    sale_line_warn = fields.Selection(WARNING_MESSAGE, 'Goods Order Line', help=WARNING_HELP, required=True, default="no-message")
    ...
    def _check_sale_product_company(self):
        ...
        if target_company:  # don't prevent writing `False`, should always work
            ...
            so_lines = self.env['goods.order.line'].sudo().search_read([('product_id', 'in', product_ids), ('company_id', '!=', target_company.id)], fields=['id', 'product_id'])
    ...
    def action_view_sales(self):
        action = self.env.ref('gio.report_all_channels_sales_action').read()[0]
        action['domain'] = [('product_tmpl_id', 'in', self.ids)]
        action['context'] = {
            'pivot_measures': ['product_uom_qty'],
            'active_id': self._context.get('active_id'),
            'active_model': 'gio.report',
