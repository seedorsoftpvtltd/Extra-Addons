...
class AccountMoveLine(models.Model):
    ...
    sale_line_ids = fields.Many2many(
        'goods.order.line',
        'goods_order_line_invoice_rel',
        'invoice_line_id', 'order_line_id',
        string='Goods Order Lines', readonly=True, copy=False)
    ...
    def _sale_create_reinvoice_sale_line(self):
        ...
        for move_line in self:
            ...
            if sale_order.state != 'sale':
                message_unconfirmed = _('The Goods Order %s linked to the Analytic Account %s must be validated before registering expenses.')
                messages = {
                    'draft': message_unconfirmed,
                    'sent': message_unconfirmed,
                    'done': _('The Goods Order %s linked to the Analytic Account %s is currently locked. You cannot register an expense on a locked Sales Order. Please create a new SO linked to this Analytic Account.'),
                    'cancel': _('The Goods Order %s linked to the Analytic Account %s is cancelled. You cannot register an expense on a cancelled Sales Order.'),
                }
            ...
            if move_line.product_id.expense_policy == 'sales_price' and move_line.product_id.invoice_policy == 'delivery':  # for those case only, we can try to reuse one
                ...
                if sale_line:  # already search, so reuse it. sale_line can be sale.order.line record or index of a "to create values" in `sale_line_values_to_create`
                    ...
                    sale_line = self.env['goods.order.line'].search([
        ...
        new_sale_lines = self.env['goods.order.line'].create(sale_line_values_to_create)
    ...
    def _sale_determine_order(self):
        ...
        if analytic_accounts:  # first, search for the open sales order
            sale_orders = self.env['goods.issue.order'].search([('analytic_account_id', 'in', analytic_accounts.ids), ('state', '=', 'sale')], order='create_date DESC')
            ...
            if analytic_accounts_without_open_order:  # then, fill the blank with not open sales orders
                sale_orders = self.env['goods.issue.order'].search([('analytic_account_id', 'in', analytic_accounts_without_open_order.ids)], order='create_date DESC')
            for sale_order in sale_orders:
                mapping[sale_order.analytic_account_id.id] = sale_order
    ...
    def _sale_prepare_sale_line_values(self, order, price):
        ...
        self.ensure_one()
        last_so_line = self.env['goods.order.line'].search([('order_id', '=', order.id)], order='sequence desc', limit=1)
