...
class CrmTeam(models.Model):
    ...
    def _compute_quotations_to_invoice(self):
        query = self.env['goods.issue.order']._where_calc([
            ('teams_id', 'in', self.ids),
            ('state', 'in', ['draft', 'sent']),
        ])
        self.env['goods.issue.order']._apply_ir_rules(query, 'read')
        _, where_clause, where_clause_args = query.get_sql()
        select_query = """
            SELECT teams_id, count(*), sum(amount_total /
                CASE COALESCE(currency_rate, 0)
                WHEN 0 THEN 1.0
                ELSE currency_rate
                END
            ) as amount_total
            FROM goods_issue_order
            WHERE %s
            GROUP BY teams_id
        """ % where_clause
        ...
        for datum in quotation_data:
            team = self.browse(datum['teams_id'])
    ...
    def _compute_sales_to_invoice(self):
        sale_order_data = self.env['goods.issue.order'].read_group([
            ('teams_id', 'in', self.ids),
            ('invoice_status','=','to invoice'),
        ], ['teams_id'], ['teams_id'])
        data_map = {datum['teams_id'][0]: datum['teams_id_count'] for datum in sale_order_data}
        for team in self:
    ...
    def action_primary_channel_button(self):
        if self._context.get('in_sales_app'):
            return self.env.ref('gio.action_order_report_so_salesteam').read()[0]
