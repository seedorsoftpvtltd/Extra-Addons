...
class GoodsIssueOrder(models.Model):
    ...
    def _default_validity_date(self):
        if self.env['ir.config_parameter'].sudo().get_param('gio.use_quotation_validity_days'):
    ...
    def _get_invoice_status(self):
        ...
        line_invoice_status_all = [
            ...
            for d in self.env['goods.order.line'].read_group([
    ...
    def get_empty_list_help(self, help):
        self = self.with_context(
            empty_list_help_document_name=_("goods order"),
        )
        return super(GoodsIssueOrder, self).get_empty_list_help(help)
        ...
        if operator == 'in' and value:
            self.env.cr.execute("""
                SELECT array_agg(so.id)
                    FROM goods_issue_order so
                    JOIN goods_order_line sol ON sol.order_id = so.id
                    JOIN goods_order_line_invoice_rel soli_rel ON soli_rel.order_line_id = sol.id
    ...
    origin = fields.Char(string='Source Document', help="Reference of the document that generated this goods order request.")
    client_order_ref = fields.Char(string='Customer Reference', copy=False)
    reference = fields.Char(string='Payment Ref.', copy=False,
        help='The payment communication of this goods order.')
    ...
    create_date = fields.Datetime(string='Creation Date', readonly=True, index=True, help="Date on which goods order is created.")
    ...
    analytic_account_id = fields.Many2one(
        ...
        states={'draft': [('readonly', False)], 'sent': [('readonly', False)]},
        domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]",
        help="The analytic account related to a goods order.")

    order_line = fields.One2many('goods.order.line', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True, auto_join=True)
    ...
    transaction_ids = fields.Many2many('payment.transaction', 'goods_issue_order_transaction_rel', 'goods_order_ids', 'transaction_id',
    ...
    def _compute_access_url(self):
        super(GoodsIssueOrder, self)._compute_access_url()
        for order in self:
    ...
    def unlink(self):
        ...
        return super(GoodsIssueOrder, self).unlink()
    ...
    def _track_subtype(self, init_values):
        ...
        if 'state' in init_values and self.state == 'sale':
            return self.env.ref('gio.mt_order_confirmed')
        elif 'state' in init_values and self.state == 'sent':
            return self.env.ref('gio.mt_order_sent')
        return super(GoodsIssueOrder, self)._track_subtype(init_values)
    ...
    def onchange_partner_id(self):
        ...
        if not self.env.context.get('not_self_saleperson') or not self.teams_id:
            values['teams_id'] = self.env['crm.team'].with_context(
                default_teams_id=self.partner_id.team_id.id
    ...
    def onchange_user_id(self):
        if self.user_id:
            self.teams_id = self.env['crm.team'].with_context(
                default_teams_id=self.teams_id.id
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            ...
            if 'company_id' in vals:
                vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code(
                    'goods.issue.order', sequence_date=seq_date) or _('New')
            else:
                vals['name'] = self.env['ir.sequence'].next_by_code('goods.issue.order', sequence_date=seq_date) or _('New')
        ...
        result = super(GoodsIssueOrder, self).create(vals)
        return result
    ...
    def _write(self, values):
        ...
        if self.env.context.get('mail_activity_automation_skip'):
            return super(GoodsIssueOrder, self)._write(values)
        ...
        if 'invoice_status' in values:
            if values['invoice_status'] == 'upselling':
                ...
                filtered_self.activity_unlink(['gio.mail_act_sale_upsell'])
                for order in filtered_self:
                    order.activity_schedule(
                        'gio.mail_act_sale_upsell',
    ...
    def copy_data(self, default=None):
        ...
        return super(GoodsIssueOrder, self).copy_data(default)
    ...
    def name_get(self):
        if self._context.get('sale_show_partner_name'):
            ...
            return res
        return super(GoodsIssueOrder, self).name_get()
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        return super(GoodsIssueOrder, self)._name_search(name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)
    ...
    def _get_invoiceable_lines(self, final=False):
        ...
        return self.env['goods.order.line'].browse(invoiceable_line_ids)
    ...
    def _create_invoices(self, grouped=False, final=False):
        ...
        if len(invoice_vals_list) < len(self):
            GoodsOrderLine = self.env['goods.order.line']
            for invoice in invoice_vals_list:
                sequence = 1
                for line in invoice['invoice_line_ids']:
                    line[2]['sequence'] = GoodsOrderLine._get_invoice_line_sequence(new=sequence, old=line[2]['sequence'])
                    sequence += 1
    ...
    def _find_mail_template(self, force_confirmation_template=False):
        ...
        if force_confirmation_template or (self.state == 'sale' and not self.env.context.get('proforma', False)):
            template_id = int(self.env['ir.config_parameter'].sudo().get_param('gio.default_confirmation_template'))
            template_id = self.env['mail.template'].search([('id', '=', template_id)]).id
            if not template_id:
                template_id = self.env['ir.model.data'].xmlid_to_res_id('gio.mail_template_sale_confirmation', raise_if_not_found=False)
        if not template_id:
            template_id = self.env['ir.model.data'].xmlid_to_res_id('gio.email_template_edi_sale', raise_if_not_found=False)
    ...
    def action_quotation_send(self):
        ...
        if template.lang:
            lang = template._render_template(template.lang, 'goods.issue.order', self.ids[0])
        ctx = {
            'default_model': 'goods.issue.order',
    ...
    def message_post(self, **kwargs):
        ...
        return super(GoodsIssueOrder, self.with_context(mail_post_autofollow=True)).message_post(**kwargs)
    ...
    def action_confirm(self):
        ...
        if self.env.user.has_group('gio.group_auto_done_settings'):
            self.action_done()
        return True
    ...
    def _notify_get_groups(self, msg_vals=None):
        ...
        groups = super(GoodsIssueOrder, self)._notify_get_groups(msg_vals=msg_vals)
    ...
    def _get_portal_return_action(self):
        ...
        self.ensure_one()
        return self.env.ref('gio.action_goods_quotations_with_onboarding')
...
class GoodsOrderLine(models.Model):
    _name = 'goods.order.line'
    _description = 'Goods Order Line'
    ...
    def write(self, values):
        ...
        result = super(GoodsOrderLine, self).write(values)
        return result

    order_id = fields.Many2one('goods.issue.order', string='Order Reference', required=True, ondelete='cascade', index=True, copy=False)
    name = fields.Text(string='Description', required=True)
    sequence = fields.Integer(string='Sequence', default=10)

    invoice_lines = fields.Many2many('account.move.line', 'goods_order_line_invoice_rel', 'order_line_id', 'invoice_line_id', string='Invoice Lines', copy=False)
    invoice_status = fields.Selection([
        ('upselling', 'Upselling Opportunity'),
        ('invoiced', 'Fully Invoiced'),
        ('to invoice', 'To Invoice'),
    ...
    analytic_tag_ids = fields.Many2many(
        ...
        domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")
    analytic_line_ids = fields.One2many('account.analytic.line', 'so_lines', string="Analytic lines")
    ...
    @api.depends('qty_delivered_method', 'qty_delivered_manual', 'analytic_line_ids.so_lines', 'analytic_line_ids.unit_amount', 'analytic_line_ids.product_uom_id')
    def _compute_qty_delivered(self):
        ...
        for so_lines in lines_by_analytic:
            so_lines.qty_delivered = mapping.get(so_lines.id or so_lines._origin.id, 0.0)
    ...
    def _get_delivered_quantity_by_analytic(self, additional_domain):
        ...
        domain = expression.AND([[('so_lines', 'in', self.ids)], additional_domain])
        data = self.env['account.analytic.line'].read_group(
            domain,
            ['so_lines', 'unit_amount', 'product_uom_id'], ['product_uom_id', 'so_lines'], lazy=False
        )

        # convert uom and sum all unit_amount of analytic lines to get the delivered qty of SO lines
        # browse so lines and product uoms here to make them share the same prefetch
        lines = self.browse([item['so_lines'][0] for item in data])
        ...
        for item in data:
            if not item['product_uom_id']:
                continue
            so_line_id = item['so_lines'][0]
            so_lines = lines_map[so_line_id]
            result.setdefault(so_line_id, 0.0)
            uom = product_uom_map.get(item['product_uom_id'][0])
            if so_lines.product_uom.category_id == uom.category_id:
                qty = uom._compute_quantity(item['unit_amount'], so_lines.product_uom, rounding_method='HALF-UP')
            else:
    ...
    def product_id_change(self):
        ...
        vals.update(name=self.get_goods_order_line_multiline_description_sale(product))
    ...
    def name_get(self):
        result = []
        for so_lines in self.sudo():
            name = '%s - %s' % (so_lines.order_id.name, so_lines.name and so_lines.name.split('\n')[0] or so_lines.product_id.name)
            if so_lines.order_partner_id.ref:
                name = '%s (%s)' % (name, so_lines.order_partner_id.ref)
            result.append((so_lines.id, name))
        return result
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        return super(GoodsOrderLine, self)._name_search(name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)
    ...
    def unlink(self):
        ...
        return super(GoodsOrderLine, self).unlink()
    ...
    def get_goods_order_line_multiline_description_sale(self, product):
        ...
        """
        return product.get_product_multiline_description_sale() + self._get_goods_order_line_multiline_description_variants()

    def _get_goods_order_line_multiline_description_variants(self):
