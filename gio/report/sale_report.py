...
class SaleReport(models.Model):
    _name = "gio.report"
    _description = "Goods Analysis Report"
    _auto = False
    _rec_name = 'date'
    _order = 'date desc'
    ...
    teams_id = fields.Many2one('crm.team', 'GIO Team', readonly=True)
    ...
    order_id = fields.Many2one('goods.issue.order', 'Order #', readonly=True)
    ...
    def _query(self, with_clause='', fields={}, groupby='', from_clause=''):
        ...
        from_ = """
                goods_order_line l
                      join goods_issue_order s on (l.order_id=s.id)
...
class SaleOrderReportProforma(models.AbstractModel):
    _name = 'report.gio.report_goodsproforma'
    _description = 'Proforma Report'

    def _get_report_values(self, docids, data=None):
        docs = self.env['goods.issue.order'].browse(docids)
        return {
            'doc_ids': docs.ids,
            'doc_model': 'goods.issue.order',
            'docs': docs,
            'proforma': True
        }
