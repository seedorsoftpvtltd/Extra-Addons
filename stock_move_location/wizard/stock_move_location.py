...
class StockMoveLocationWizard(models.TransientModel):
    ...
    owner_id = fields.Many2one(comodel_name="res.partner", string="Customer")
    ...
    @api.onchange("origin_location_id", "owner_id")
    ...
    def _get_group_quants(self):
        ...
        query = """
                        SELECT product_id, lot_id, owner_id, package_id, 
                            SUM(quantity) AS quantity, SUM(reserved_quantity) AS reserved_quantity
                        FROM stock_quant
                        WHERE location_id = %s AND owner_id = %s
                        GROUP BY product_id, lot_id, owner_id, package_id
                    """
        self.env.cr.execute(query, (location_id.id, owner_id.id))
    ...
    #         query = """
    #             SELECT product_id, lot_id, owner_id, package_id, SUM(quantity) AS quantity,
    #                 SUM(reserved_quantity) AS reserved_quantity
    #             FROM stock_quant
    #             WHERE location_id = %s AND owner_id = %s AND product_id IN %s
    #             GROUP BY product_id, lot_id, owner_id, package_id
    #         """
    #         self.env.cr.execute(query, (location_id.id,owner_id.id, products,))
    #         return self.env.cr.dictfetchall()
    #     else:
    #         query = """
    #                         SELECT product_id, lot_id, owner_id, package_id, SUM(quantity) AS quantity,
    ...
    def _get_stock_move_location_lines_values(self):
        ...
        for group in self._get_group_quants():
            print(group)
            if self.product_id:
                product_ids = [p.id for p in self.product_id]
                if group['product_id'] in product_ids:
