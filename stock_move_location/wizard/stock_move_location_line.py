...
class StockMoveLocationWizardLine(models.TransientModel):
    ...
    move_location_wizard_id = fields.Many2one(
        string="Move location Wizard", comodel_name="wiz.stock.move.location",
    )
    ...
    result_package_id = fields.Many2one("stock.quant.package", string="Result Package", related='package_id',
                                        store=True)
    ...
    def create_move_lines(self, picking, move):
        for line in self:
            ...
            self.lot_id.compute_internal_qties()
        return True
    ...
    def _get_move_line_values(self, picking, move):
        ...
        self.lot_id.write({'internal_transfer': True})
        return {
            "product_id": self.product_id.id,
...
#
#     def pallet_check(self):
#         for rec in self:
#             if not rec.package_id and rec.lot_id:
