...
class InsGeneralLedger(models.TransientModel):
    ...
    def build_detailed_move_lines(self, offset=0, account=0, fetch_range=FETCH_RANGE):
        ...
        sql = ('''
                ...
                WHERE %s
                GROUP BY l.id, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.name, m.id, m.name, c.rounding, cc.id, cc.rounding, cc.position, c.position, c.symbol, cc.symbol, p.name, j.type, am.narration
                ORDER BY %s
                OFFSET %s ROWS
                FETCH FIRST %s ROWS ONLY
    ...
    def process_data(self):
        ...
        for account in account_ids:
            ...
            _logger.info(current_lines)
#            raise UserError(_(sql))
