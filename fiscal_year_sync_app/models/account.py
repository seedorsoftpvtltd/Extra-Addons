        ...
        for record in self:
            if record.invoice_date or record.date:
                ...
                period_ids = self.env['account.period'].search(
                    [('date_start', '<=', today), ('date_stop', '>=', today)], limit=1)
...
class AccountJournal(models.Model):
    ...
    type = fields.Selection(selection_add=[('situation', 'Opening/Closing Situation')], required=True,
...
class AccountMove(models.Model):
    ...
    def check_period(self):
        fiscalyear_ids = self.env['account.fiscalyear'].search([('state', '=', 'draft')])
        for record in self:
    ...
    # 	if record.date < fiscal.date_start or record.date > fiscal.date_stop:
    # 		raise UserError(_('You can not create journal entries because your date not in this fiscalyear %s') % (fiscal.name))
    ...
    def _query_get(self, domain=None):
        ...
        if not context.get('fiscalyear'):
            if context.get('all_fiscalyear'):
                ...
                domain += ['|', ('fiscalyear_id', 'in', fiscalyear_ids.ids), ('fiscalyear_id', '=', False)]
            else:
