...
class ProductSummary(models.Model):
    ...
    def _duration(self):
        for rec in self:
            if rec.out_date and rec.in_date:
                rec['duration'] = (abs((fields.Datetime.to_datetime(rec.out_date).date()
                                       - fields.Datetime.to_datetime(rec.in_date).date()).days) + 1)
            else:
                rec['duration'] = 1
    ...
    @api.depends('product_id.storage_type')
    def _storage_product(self):
        for rec in self:
            print(rec.product_id.storage_type, '--------------rec.product_id.storage_type--------------')
    ...
    def _duration(self):
        for rec in self:
            if rec.end_date and rec.start_date:
                rec['duration'] = (abs((fields.Datetime.to_datetime(rec.end_date).date()
                                       - fields.Datetime.to_datetime(rec.start_date).date()).days) +1)
            else:
                rec['duration'] = 1
