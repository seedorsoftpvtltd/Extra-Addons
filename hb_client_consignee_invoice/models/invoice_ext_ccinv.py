...
class Partnerforminherit(models.Model):
    ...
    def create_ccinv_invoice_coree(self, parent_partner):
        parent_partner._inv_gen_date()
        invoice_type = self.env['ir.config_parameter'].sudo().get_param(
            'hb_wms_invoice_v1.invoice_type')
        if invoice_type == 'con':
            return self.create_inv_invoice_core_consolidatedccinv(parent_partner)
        elif invoice_type == 'sep':
            return self.create_inv_invoice_core_seperateccinv(parent_partner)
        else:
            raise ValidationError(_('Please configure the Invoice Type from the Settings'))

    def create_inv_invoice_core_consolidatedccinv(self, parent_partner):
        invoices = []
        for partner in self:
            parent_partner = parent_partner
            parent_partner._inv_gen_date()
            s = parent_partner.last_inv_date
            e = parent_partner.inv_end_date
            ee = datetime.strptime(str(e), '%Y-%m-%d %H:%M:%S')
            ss = datetime.strptime(str(s), '%Y-%m-%d %H:%M:%S')
            agreement_id = partner.env['agreement'].search([('partner_id', '=', parent_partner.id)])
            ...
            inven = self.env['product.summary'].search(
                [('consignee', '=', partner.id), ('storage_product', '=', False),('gen_seperate_inv','=',True)])
            ...
            picking_id_in = self.env['stock.picking'].search(
                [('picking_type_id.code', '=', 'incoming'), ('x_consignee', '=', partner.id),
            ...
            picking_id_out = self.env['stock.picking'].search(
                [('picking_type_id.code', '=', 'outgoing'),  ('x_consignee', '=', partner.id),
                 ('scheduled_date', '>=', s), ('product_id.storage_type', '=', False)])
            ...
            if agreement_id and invoice_line_list:
                invoice = partner.env['account.move'].create({
                    ...
                    'start_date_sto': s,
                    'end_date_sto': e,
                    'invoice_origin': parent_partner.name,
                    'invoice_user_id': current_user,
                    'narration': parent_partner.name,
                    'partner_id': parent_partner.id,
                    'x_consignee1': partner.id,
                    'currency_id': parent_partner.env.user.company_id.currency_id.id,
                    # 'journal_id': int(UOMer_journal_id),
                    'invoice_payment_ref': parent_partner.name,
            ...
            for sty in sto_type:
                ...
                tinven = self.env['product.summary'].search(
                    [('consignee', '=', partner.id), ('storage_product', '=', True),
                     ('product_id.storage_type.name', '=', sty),('gen_seperate_inv','=',True)])
            ...
            if agreement_id and tinvoice_line_list:
                tinvoice = self.env['account.move'].create({
                    'type': 'out_invoice',
                    'sto_type': 'warehouse',
                    'start_date_sto': s,
                    'end_date_sto': e,
    ...
    def create_inv_invoice_core_seperateccinv(self, parent_partner):
        invoices = []
        for partner in self:
            ...
            storage_charges = partner.env['agreement.charges'].search(
                ['&', ('charge_type', '=', 'storage'), ('agreement_id', '=', agreement_id.id),
                 ('storage_type', '=', False)])
            print(agreement_id, 'agreement_id',storage_charges, 'storage_charges')
            quantity_dict = {}
            charge_unit_types = {}
            for c in storage_charges:
                chrg = c['charge_unit_type']['name']
