...
class HrEmployeeDocument(models.Model):
    ...
    doc_attachment_ids = fields.Many2many('ir.attachment', 'doc_attach_rels', 'doc_id', 'attach_id3', string="Attachment",
...
class HrEmployee(models.Model):
    ...
    def _document_count1(self):
        for each in self:
            document_ids = self.env['hr.employee.documents'].search([('employee_ref', '=', each.id)])
            each.document_count1 = len(document_ids)


    def document_view1(self):
        self.ensure_one()
        domain = [
        ...
        return {
            'name': _('Documents'),
            'domain': domain,
            'res_model': 'hr.employee.documents',
    ...
    document_count1 = fields.Integer(compute='_document_count1', string='# Documents')


class HrEmployeeAttachment(models.Model):
    _inherit = 'ir.attachment'

    doc_attach_rels = fields.Many2many('hr.employee.documents', 'doc_attachment_ids', 'attach_id3', 'doc_id',
