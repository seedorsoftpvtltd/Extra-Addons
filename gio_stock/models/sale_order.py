...
class SaleOrder(models.Model):
    ...
    picking_ids = fields.One2many('stock.picking', 'goods_id', string='Transfers')
    ...
    def _onchange_company_id(self):
        if self.company_id:
            warehouse_id = self.env['ir.default'].get_model_defaults('goods.issue.order').get('warehouse_id')
    ...
    def _log_decrease_ordered_quantity(self, documents, cancel=False):
        ...
        def _render_note_exception_quantity_so(rendering_context):
            ...
            order_line_ids = self.env['goods.order.line'].browse([order_line.id for order in order_exceptions.values() for order_line in order[0]])
            ...
            return self.env.ref('gio_stock.exception_on_so').render(values=values)
...
class SaleOrderLine(models.Model):
    ...
    move_ids = fields.One2many('stock.move', 'goods_line_id', string='Stock Moves')
    ...
    def _compute_qty_at_date(self):
        ...
        grouped_lines = defaultdict(lambda: self.env['goods.order.line'])
    ...
    def create(self, vals_list):
        ...
        # commanded below line for import feature
        # lines.filtered(lambda line: line.state == 'sale')._action_launch_stock_rule()
        return lines

    def write(self, values):
        lines = self.env['goods.order.line']
    ...
    def _prepare_procurement_values(self, group_id=False):
        ...
        date_planned = self.order_id.date_order\
            + timedelta(days=self.customer_lead or 0.0) - timedelta(days=self.order_id.company_id.security_leads)
        values.update({
            'group_id': group_id,
            'goods_line_id': self.id,
        ...
        for line in self.filtered("order_id.commitment_date"):
            date_planned = fields.Datetime.from_string(line.order_id.commitment_date) - timedelta(days=line.order_id.company_id.security_leads)
            values.update({
