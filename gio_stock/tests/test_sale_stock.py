...
class TestSaleStock(TestSale):
    def _get_new_sale_order(self, amount=10.0):
        ...
        sale_order = self.env['goods.issue.order'].create(sale_order_vals)
        return sale_order
    ...
    def test_00_sale_stock_invoice(self):
        ...
        self.so = self.env['goods.issue.order'].create({
        ...
        self.so.flush()
        for field in self.env['goods.order.line']._fields.values():
    ...
    def test_02_sale_stock_return(self):
        ...
        self.so = self.env['goods.issue.order'].create(so_vals)
    ...
    def test_05_confirm_cancel_confirm(self):
        ...
        so1 = self.env['goods.issue.order'].create({
            'partner_id': partner1,
            'order_line': [(0, 0, {
                'name': item1.name,
                'product_id': item1.id,
    ...
    def test_09_qty_available(self):
        ...
        so = self.env['goods.issue.order'].create({
            'partner_id': self.partner.id,
            'order_line': [
                (0, 0, {'name': item1.name, 'product_id': item1.id, 'product_uom_qty': 1, 'product_uom': item1.uom_id.id, 'price_unit': item1.list_price}),
            ],
