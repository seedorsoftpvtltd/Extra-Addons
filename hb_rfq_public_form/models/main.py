    ...
    def purchase_comparison(self, purchase_requisition_id, **post):
                ...
                if values:
                    if line.product_id.id not in product_ids:
                        ...
                        values.append({
                            ...
                            'confirm': line.confirm  # Added 'confirm' field
                        })
                else:
                    product_ids.append(line.product_id.id)
                    values.append({
        ...
        for separate_values in values:
            for suppliers in supplier_ids:
                for record in request.env['purchase.order'].sudo().search(
                        [('requisition_id', '=', purchase_requisition_id.id), ('partner_id', '=', suppliers['supplier_id'])]):
                    for po_line in request.env['purchase.order.line'].search(
                            [('order_id', '=', record.id), ('product_id', '=', separate_values['product_id'])]):
        ...
        return request.render('purchase_comparison_chart.purchase_comparison', {
            'data': values,
            'supplier': supplier_ids,
            'purchase_requisition_id': purchase_requisition_id,
            'number': number,
            'to_no': total_no,
            'column_no': even_number,
            'supplier_amount_total': supplier_amount_total,
            'supplier_amount_total_1': supplier_amount_total_1,
            'odd_number': odd_number,
        })

    @http.route('/update_confirm', type='json', auth="public", methods=['POST'], website=True)
    def update_confirm(self, purchase_order_line_id, confirm, **post):
        request.env['purchase.order.line'].browse(int(purchase_order_line_id)).write({'confirm': confirm})
        return True
