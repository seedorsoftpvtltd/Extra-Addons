...
# 		lines = self.final_vals_to_lines(final_vals, level, html_formating)
	...
	def _get_html(self):
		...
		result['html'] = self.env.ref('account_parent.report_coa_hierarchy').render(rcontext)
		return result
	...
	@api.model
	def create_fiscal_year_menu(self):
		""" Creates menu for fiscal year"""
		fiscal_action = self.env.ref('account.actions_account_fiscal_year')
		if not self.env['ir.ui.menu'].search([('action', '=', 'ir.actions.act_window,%s' % fiscal_action.id)]):
			vals = {'name': 'Fiscal Years',
					'parent_id': self.env.ref('account.account_account_menu').id,
					'action': 'ir.actions.act_window,%s' % fiscal_action.id,
					'groups_id': [(6, 0, [self.env.ref('account.group_fiscal_year').id])]
					}	
			self.env['ir.ui.menu'].create(vals)
