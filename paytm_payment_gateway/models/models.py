...
class PaymentAcquirerAtom(models.Model):
    ...
    def __encode__(self,to_encode ,iv ,key):
        ...
        # Encrypt
        c=AES.new (str(key).encode("utf8") ,AES.MODE_CBC ,str(iv).encode("utf8"))
        to_encode=c.encrypt (str(to_encode).encode("utf8"))
        # Encode
    ...
    def __decode__(self,to_decode ,iv ,key):
        ...
        # Decrypt
        c=AES.new (str(key).encode("utf8") ,AES.MODE_CBC ,str(iv).encode("utf8"))
        to_decode=c.decrypt (str(to_decode).encode("utf8"))
