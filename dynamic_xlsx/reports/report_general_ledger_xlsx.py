...
class InsGeneralLedgerXlsx(models.AbstractModel):
    ...
    def prepare_report_contents(self, data, acc_lines, filter):
        ...
        self.row_pos += 3
        a=self.convert_to_date(filter.get('date_from'))
        from_date = a.strftime('%d/%m/%Y')
        b = self.convert_to_date(filter.get('date_to'))
        to_date = b.strftime('%d/%m/%Y')

        self.sheet.write_string(self.row_pos, 2, str(from_date),self.format_header)
        self.sheet.write_string(self.row_pos, 3, _(' To '),
                                self.format_merged_header_without_border)
        self.sheet.write_string(self.row_pos, 4, str(to_date),self.format_header)
        self.row_pos += 3
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        if record:
            ...
            filters, account_lines = record.with_context(company_id=record.company_id.id).get_report_datas()
            # Filter section
