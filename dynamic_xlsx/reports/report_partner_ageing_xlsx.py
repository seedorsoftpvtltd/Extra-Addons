...
class InsPartnerAgeingXlsx(models.AbstractModel):
    ...
    def prepare_report_contents(self, data, period_dict, period_list, ageing_lines, filter):
        data = data[0]
        self.row_pos += 3
        a = self.convert_to_date(filter.get('as_on_date'))
        as_on = a.strftime('%d/%m/%Y')

        self.sheet.write_string(self.row_pos, 3, str(as_on), self.format_header)
        self.sheet.write_string(self.row_pos, 2, _(' As on Date'),
