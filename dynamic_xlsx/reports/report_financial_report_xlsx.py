...
class InsFinancialReportXlsx(models.AbstractModel):
    ...
    def _define_formats(self, workbook):
        ...
        self.line_header1 = workbook.add_format({
            'bold': True,
            'font_size': 10,
            'align': 'right',
            'font': 'Arial',
        ...
        self.right_aligned_format = workbook.add_format({'align': 'right'})
        self.left_aligned_format = workbook.add_format({'align': 'left'})
    ...
    def prepare_report_contents(self, data):
        ...
        a = self.convert_to_date(data['form']['date_from'])

        from_date = a.strftime('%d/%m/%Y')
        b = self.convert_to_date(data['form']['date_to'])
        to_date = b.strftime('%d/%m/%Y')

        self.sheet.write_string(self.row_pos, 1,str(from_date), self.line_header1)
        self.sheet.write_string(self.row_pos, 2, _(' To '),
                                self.format_merged_header_without_border)
        self.sheet.write_string(self.row_pos, 3, str(to_date), self.format_header)
        self.row_pos += 3
