...
class InvoiceApproval(models.Model):
    ...
    payment_approver_ids = fields.Many2many('res.users', 'payment_ids', string='Customer Payment Approver', domain=lambda self: [
        ('groups_id', 'in', self.env.ref('invoice_multi_approval.group_approver').id)],help='In this field you can add the approvers for the Customer Payment')
    approve_vendor_payment=fields.Boolean(string='Approval on Vendor Payment',help='Enable this field for adding the approvals for the Customer Payment')

    vendor_payment_approver_ids = fields.Many2many('res.users', 'vendor_payment_ids', string='Vendor Payment Approver', domain=lambda self: [
        ('groups_id', 'in', self.env.ref('invoice_multi_approval.group_approver').id)],help='In this field you can add the approvers for the Vendor Payment')
