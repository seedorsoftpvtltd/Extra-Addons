...
class Testwarehouse(SavepointCase):
    @classmethod
    def setUpClass(cls):
        super(Testwarehouse, cls).setUpClass()
    ...
    def test_date_planned_1(self):
        ...
        """
        po = Form(self.env['warehouse.order'])
    ...
    def test_warehouse_order_sequence(self):
        warehouseOrder = self.env['warehouse.order'].with_context(tracking_disable=True)
        company = self.env.user.company_id
        self.env['ir.sequence'].search([
            ('code', '=', 'warehouse.order'),
        ]).write({
            'use_date_range': True, 'prefix': 'WO/%(range_year)s/',
        })
        vals = {
        ...
        warehouse_order = warehouseOrder.create(vals.copy())
        self.assertTrue(warehouse_order.name.startswith('PO/2019/'))
        vals['date_order'] = '2020-01-01'
        warehouse_order = warehouseOrder.create(vals.copy())
        self.assertTrue(warehouse_order.name.startswith('PO/2020/'))
        # In EU/BXL tz, this is actually already 01/01/2020
        vals['date_order'] = '2019-12-31 23:30:00'
        warehouse_order = warehouseOrder.with_context(tz='Europe/Brussels').create(vals.copy())
        self.assertTrue(warehouse_order.name.startswith('PO/2020/'))
