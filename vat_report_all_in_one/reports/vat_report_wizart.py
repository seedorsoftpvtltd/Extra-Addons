...
class VATReportWizard(models.TransientModel):
    ...
    opening_balance_included = fields.Boolean('Include Opening')
    ...
    def add_company_address(self, sheet, workbook):
        row = 3
        company_format = workbook.add_format({'bold': 0, 'border': 0, 'valign': 'vcenter', 'fg_color': '#17375e', 'font_size': 11})
        company_format.set_font_color('white')
        company_id = self.env.user.company_id
        sheet.merge_range('A{row}:D{row}'.format(row=row), company_id.name, company_format)
        row += 1
        if company_id.street:
            sheet.merge_range('A{row}:D{row}'.format(row=row), company_id.street, company_format)
            row += 1
        if company_id.street2:
            sheet.merge_range('A{row}:D{row}'.format(row=row), company_id.street2, company_format)
            row += 1
        if company_id.city or company_id.state_id or company_id.zip:
            ...
            sheet.merge_range('A{row}:D{row}'.format(row=row), addr, company_format)
            row += 1
        if company_id.country_id:
            sheet.merge_range('A{row}:D{row}'.format(row=row), company_id.country_id.name, company_format)
        return row
    ...
    def get_lines(self, options):
        ...
        for tax in taxes.values():
            for tax_group_id in tax_group_ids:
                if tax['net'] and tax['tax_group_id'] == tax_group_id:
    ...
    def get_opening_balance(self, data):
        ...
        query = """
            SELECT 
                SUM(aml.debit - aml.credit) as balance FROM account_move_line as aml
                LEFT JOIN account_account ac ON (aml.account_id = ac.id)
                LEFT JOIN account_move am ON (aml.move_id = am.id)
            WHERE 
                ac.is_a_vat_account = true AND 
                aml.date < '%s' AND
                (aml.exclude_from_invoice_tab = true OR am.type = 'entry')
        """ % date_from
    ...
    def generate_xlsx_report(self, data):
        ...
        bold = workbook.add_format({'bold': True, 'align': 'center', 'valign': 'vcenter', 'fg_color': '#17375e', 'border': 1})
        style1 = workbook.add_format({'bold': True, 'font_size': 11, 'fg_color': '#17375e'})
        style2 = workbook.add_format({'bold': True, 'font_size': 11, 'fg_color': '#558ed5'})
        money = workbook.add_format({'num_format': '_(* #,##0.00_);_(* (#,##0.00);_(* "-"??_);_(@_)'})
        date_format = workbook.add_format({'num_format': 'm/d/yyyy'})
        money_bold = workbook.add_format({'num_format': '_(* #,##0.00_);_(* (#,##0.00);_(* "-"??_);_(@_)', 'bold': True, 'fg_color': '#17375e'})
        money_style1 = workbook.add_format({'num_format': '_(* #,##0.00_);_(* (#,##0.00);_(* "-"??_);_(@_)', 'bold': False, 'fg_color': '#558ed5'})
        merge_format = workbook.add_format({
            'bold': 1, 'border': 1, 'align': 'center', 'valign': 'vcenter', 'fg_color': '#17375e', 'font_size': 11})
        merge_format.set_font_color('white')
        sheet.merge_range('A1:D1', report_name, merge_format)
        row = self.add_company_address(sheet,workbook)
        row += 2
    ...
    def generate_vat_return_report(self, sheet, workbook, output, row, col, bold, style1, style2, money_style1, money, money_bold, data):
        opening_money = workbook.add_format({'num_format': '_(* #,##0.00_);_(* (#,##0.00);_(* "-"??_);_(@_)', 'bold': True, 'fg_color': '#F4F5C9'})
        opening_bold = workbook.add_format({'bold': True, 'fg_color': '#F4F5C9', 'border': 1})
        bold.set_font_color('white')
        style1.set_font_color('white')
        money_bold.set_font_color('white')
        style2.set_font_color('white')
        money_style1.set_font_color('white')
        results = self.get_lines(data)
        opening_balance = 0
        if self.opening_balance_included:
            opening_balance = self.get_opening_balance(data)
        ...
        if opening_balance and self.opening_balance_included:
            sheet.write(row, col, 'Opening Balance', opening_bold)
            sheet.write(row, col + 1, '', opening_bold)
            sheet.write(row, col + 2, '', opening_bold)
            sheet.write(row, col+3, opening_balance, opening_money)
        row += 1
        ...
        for group in results:
            ...
            for tax_group in groups:
                ...
                if taxes:
                    ...
                    for tax in taxes:
                        if group == 'sale':
                            total_net_amount += tax.get('net') * -1
                            total_tax_amount += tax.get('tax') * -1
                            sheet.write(row, col, tax.get('name'))
                            sheet.write(row, col + 1, tax.get('net') * -1, money)
                            sheet.write(row, col + 3, tax.get('tax') * -1, money)
                        else:
                    ...
                    sheet.write(group_row, group_col + 1, formula1, money_style1)
                    formula2 = '=SUM(D{}:D{})'.format(group_row + 2, row)
                    sheet.write(group_row, group_col + 3, formula2, money_style1)
            # Total Tax type
            ...
            # else:
            #     total_tax_payable_refundable -= total_tax_amount
        total_tax_payable_refundable += opening_balance
        ...
        if data.get('tax_payment_included', False):
            ...
            sheet.write(row, col, 'Add: TAX Payment')
        ...
        if data.get('tax_refund_included', False):
            ...
            sheet.write(row, col, 'Less: TAX Refund')
        ...
        sheet.write(row, col + 3, balance_vat_payable, money_bold)
        sheet.conditional_format('A{}:D{}'.format(row + 1, row + 1), {
            'type': 'formula', 'criteria': 'True',
            'format': workbook.add_format({'bottom': 1, 'top': 1})
        })
    ...
    def generate_vat_summary_report(self, sheet, workbook, output, row, col, bold, style1, style2, money, money_bold, data, date_format):
        style11 = workbook.add_format({'border': 1,'bold': True, 'align': 'center', 'valign': 'vcenter', 'font_size': 11, 'fg_color': '#17375e'})
        style11.set_font_color('white')
        style11.set_text_wrap()
        style11.set_border_color('white')
        money_bold.set_font_color('white')
        results = self.get_lines_statement(data)
        sheet.write(row - 1, col, 'Date', style11)
        sheet.write(row - 1, col + 1, 'Partner', style11)
        sheet.write(row - 1, col + 2, 'Tax Name', style11)
        # sheet.write(row - 1, col + 3, 'Tax Type', style1)
        sheet.write(row - 1, col + 3, 'Document Type', style11)
        sheet.write(row - 1, col + 4, 'Document No', style11)
        sheet.write(row - 1, col + 5, 'Tax No', style11)
        sheet.write(row - 1, col + 6, 'Description', style11)
        sheet.write(row - 1, col + 7, 'Amount without VAT', style11)
        # sheet.write(row - 1, col + 9, 'Amount without VAT', style1)
        sheet.write(row - 1, col + 8, 'VAT Amount', style11)
        sheet.write(row - 1, col + 9, 'Amount with VAT', style11)
        sheet.conditional_format('A{}:J{}'.format(row, row), {
            'type': 'formula', 'criteria': 'True',
        ...
        for res in results:
            ...
            if res.get('tax_type') == 'sale':
                sign = -1
            if res.get('tax_type') == 'purchase':
                sign = 1
            ...
            sheet.write(row, col + 2, res.get('tax_name'))
            # sheet.write(row, col + 3, TAX_TYPE[res.get('tax_type')])
            sheet.write(row, col + 3, DOCUMENT_TYPE[res.get('type')])
            sheet.write(row, col + 4, res.get('name'))
            sheet.write(row, col + 5, res.get('tax_number', '') or '')
            sheet.write(row, col + 6, description)
            sheet.write(row, col + 7, net, money)
            # sheet.write(row, col + 9, amount_untaxed, money)
            sheet.write(row, col + 8, tax, money)
            sheet.write(row, col + 9, f'=SUM(H{row+1}:I{row+1})', money)
            row += 1

        sheet.conditional_format('I{}:J{}'.format(row, row), {
            'type': 'formula', 'criteria': 'True',
            'format': workbook.add_format({'bottom': 2})
        })
        net_formula = "=SUM(H{}:H{})".format(start_row + 1, row)
        sheet.write(row, col + 7, net_formula, money_bold)
        # untaxed_formula = "=SUM(J{}:J{})".format(start_row + 1, row)
        # sheet.write(row, col + 9, untaxed_formula, money_bold)
        vat_formula = "=SUM(I{}:I{})".format(start_row + 1, row)
        sheet.write(row, col + 8, vat_formula, money_bold)
        total_formula = "=SUM(J{}:J{})".format(start_row + 1, row)
        sheet.write(row, col + 9, total_formula, money_bold)
