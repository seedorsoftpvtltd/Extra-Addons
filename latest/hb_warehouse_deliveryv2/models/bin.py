    ...
    def bin_allocc(self):
        ...
        for rec in self:
            ...
            if rec.state in ('done', 'cancel'):
                raise UserError(_("You can't edit the completed Transfer!!"))
            ...
            if rec.state not in ('done', 'cancel'):
                ...
                for prod in rec.move_ids_without_package:
                    # if prod.product_id.tracking:
                    #     raise UserError(_(prod.product_id.tracking))
                    # print(prod.product_id.tracking, '-------------------prod.product_id.tracking--------------')

                    if prod.product_id.tracking == 'serial':
                        print('--------------Product - Track by Serial Number-----------')
                        ...
                        for b in packer.bins:
                            ...
                            #     print("====> ", item.string())
                            # raise UserError(_(b.items))
                            print("UNFITTED ITEMS:")
                            for item in b.unfitted_items:
                                print("====> ", item.string())
                              #  raise UserError(_('unfitted 1'))
                            for item in b.items:
                              #  raise UserError(_('fitted 1'))
                        ...
                        for i in buno:
                            ...
                            for i in range(i[2]):
                                mov_line = prod.move_line_ids
                                # print(mov_line, '//////////////////////////////////////////////////////////')
                                for line in mov_line:
                                    # raise UserError(_("Herlineeeeee"))

                                    line['result_package_id'] = int(pack)
                                    # line['product_uom_qty'] = 1
                                    line['location_dest_id'] = location.id
                                    line['picking_id'] = rec.id
                                    line['move_id'] = int(move)
                        ...
                        print('elseeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee')
                        ...
                        for b in packer.bins:
                            ...
                            # print("UNFITTED ITEMS:")
                            for item in b.unfitted_items:
                                #raise UserError(_('unfitted 2'))
                                print("====> ", item.string())
                            # raise UserError(_(b.items))

                            for item in b.items:
#                                raise UserError(_('fitted 2'))
                                print("FITTED ITEMS", item)
                                pallet = b.string().split('(')[0]
                                move = item.string().split('(')[0]
                                qt = 0
                                pack1.append(pallet)
                        ...
                        for i in buno:
                            ...
                            moveline = self.env['stock.move.line'].create(vals)
#                            raise UserError(_("Breeseeeeeeee"))

                            moveline.lot_auto_create()
                            print(moveline, 'movelineeeeeeeeeeeeeeeeeeeeeeeee')
