...
class BalanceSheetView(models.TransientModel):
    ...
    def view_report(self, option, tag):
        ...
        return {
            ...
            'currency': currency,
            'balance_sheet_lines': final_report_lines,
        }
    ...
    def get_dynamic_xlsx_report(self, options, response, report_data,
        ...
        format_2 = workbook.add_format(
            {'bold': True, 'font_size': '10px', 'border_color': 'black', 'border': 1, 'left': 0,
             'right': 0,
             })
        format_3 = workbook.add_format(
            {'bold': True, 'font_size': '10px', 'border_color': 'black', 'border': 1, 'left': 0, 'align': 'right'
        ...
        format_5 = workbook.add_format(
            {'bold': True, 'font_size': '10px', 'border_color': 'black', 'border': 1, 'bottom': 0, 'top': 0, 'left': 0,
             'align': 'right'

             })
        format_6 = workbook.add_format(
            {'bold': True, 'font_size': '10px', 'border_color': 'black', 'border': 1, 'bottom': 0, 'top': 0, 'right': 0

             })
        format_4 = workbook.add_format({'bold': True, 'font_size': '10px'})
        format_7 = workbook.add_format({'font_size': '8px'})
        format_10 = workbook.add_format(
            {'font_size': '8px', 'border_color': 'black', 'border': 1, 'bottom': 0, 'top': 0, 'right': 0})
        format_8 = workbook.add_format({'font_size': '8px', 'align': 'right'})
        format_9 = workbook.add_format(
            {'font_size': '8px', 'border_color': 'black', 'border': 1, 'bottom': 0, 'top': 0, 'left': 0,
             'align': 'right'
        ...
        if lines:
            for line in lines:
                ...
                if not line['p_id']:
                    for child_line in lines:
                        if line['r_id'] == child_line['p_id'] and float(child_line['balance']) > 0:
                            sheet.write(row, col, child_line['name'], format_2)
            col += 1
            sheet.write(row, col, ' ', format_2)
            col += 1
            sheet.write(row, col, ' ', format_3)
            col += 1
            for line in lines:
                sheet.set_row(row, 20)
                if not line['p_id']:
                ...
                for line in lines:
                    if not line['p_id']:
                        for child_line in lines:
                            if line['r_id'] == child_line['p_id'] and float(child_line['balance']) > 0:
                                sub_row += 1
                                sheet.set_row(sub_row, 20)
                                col = 0
                                sheet.write(sub_row, col, child_line['name'], format_4)
                                col += 2
                                sheet.write(sub_row, col, child_line['m_balance'], format_5)
                                if child_line['r_id']:
                                    for sub_child_lines in lines:
                                        if child_line['r_id'] == sub_child_lines['p_id']:
                                            sub_row += 1
                                            sheet.set_row(sub_row, 20)
                                            col = 0
                                            sheet.write(sub_row, col, sub_child_lines['name'], format_7)
                                            col += 1
                                            sheet.write(sub_row, col, sub_child_lines['m_balance'], format_8)
                                            col += 1
                                            sheet.write(sub_row, col, '', format_9)
                                            if sub_child_lines['r_id']:
                                                for sub_line in lines:
                                                    if sub_child_lines['r_id'] == sub_line['p_id']:
                                                        sub_row += 1
                                                        sheet.set_row(sub_row, 20)
                                                        col = 0
                                                        sheet.write(sub_row, col, sub_line['name'], format_7)
                                                        col += 1
                                                        sheet.write(sub_row, col, sub_line['m_balance'], format_8)
                                                        col += 1
                                                        sheet.write(sub_row, col, '', format_9)
                ...
                for line in lines:
                    if not line['p_id']:
                        for child_line in lines:
                            if line['r_id'] == child_line['p_id'] and float(child_line['balance']) < 0:
                                child_row += 1
                                sheet.set_row(child_row, 20)
                                col = 3
                                sheet.write(child_row, col, child_line['name'], format_6)
                                col += 2
                                sheet.write(child_row, col, child_line['m_balance'], format_5)
                                if child_line['r_id']:
                                    for sub_child_lines in lines:
                                        if child_line['r_id'] == sub_child_lines['p_id']:
                                            child_row += 1
                                            sheet.set_row(child_row, 20)
                                            col = 3
                                            sheet.write(child_row, col, sub_child_lines['name'], format_10)
                                            col += 1
                                            sheet.write(child_row, col, sub_child_lines['m_balance'], format_8)
                                            col += 1
                                            sheet.write(child_row, col, '', format_9)
                                            if sub_child_lines['r_id']:
                                                for sub_line in lines:
                                                    if sub_child_lines['r_id'] == sub_line['p_id']:
                                                        child_row += 1
                                                        sheet.set_row(child_row, 20)
                                                        col = 3
                                                        sheet.write(child_row, col, sub_line['name'], format_10)
                                                        col += 1
                                                        sheet.write(child_row, col, sub_line['m_balance'], format_8)
                                                        col += 1
                                                        sheet.write(child_row, col, '', format_9)
            if sub_row > child_row:
                while child_row < sub_row:
                    child_row += 1
                    sheet.set_row(child_row, 20)
                    col = 3
                    sheet.write(child_row, col, ' ', format_10)
                    col += 1
                    sheet.write(child_row, col, ' ', format_8)
                    col += 1
                    sheet.write(child_row, col, '', format_9)
            elif sub_row < child_row:
                while sub_row < child_row:
                    sub_row += 1
                    sheet.set_row(sub_row, 20)
                    col = 0
                    sheet.write(sub_row, col, ' ', format_7)
                    col += 1
                    sheet.write(sub_row, col, ' ', format_8)
                    col += 1
                    sheet.write(sub_row, col, '', format_9)
            row = sub_row + 1
            sheet.set_row(row, 20)
            col = 0
            sheet.write(row, col, 'Total ', format_2)
            col += 1
            sheet.write(row, col, ' ', format_2)
            col += 1
            for line in lines:
                if not line['p_id']:
                    for child_line in lines:
                        if line['r_id'] == child_line['p_id'] and float(child_line['balance']) > 0:
                            sheet.write(row, col, child_line['m_balance'], format_3)
            col += 1
            sheet.write(row, col, 'Total ', format_2)
            col += 1
            sheet.write(row, col, '', format_2)
            ...
            for line in lines:
                if not line['p_id']:
                    for child_line in lines:
                        if line['r_id'] == child_line['p_id'] and float(child_line['balance']) < 0:
                            sheet.write(row, col, child_line['m_balance'], format_11)
        # sheet.write(row, col, '', sub_heading)
        # sheet.write(row, col + 1, 'Debit', sub_heading)
        # sheet.write(row, col + 2, 'Credit', sub_heading)
        # sheet.write(row, col + 3, 'Balance', sub_heading)
