    ...
    def _get_report_values(self, docids, data=None):
        ...
        if docids:
            invoices = inv_obj.search(
                [("operation_id", "in", docids), ("type", "=", "out_invoice")]
            )
            docs = self.env["freight.operation"].browse(docids)
        if not invoices:
            raise UserError(_("Current Shipping order have no Invoice!!"))
        docargs = {
            "docids": docids,
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if docids:
            bills = inv_obj.search(
                [("operation_id", "in", docids), ("type", "=", "in_invoice")]
            )
            docs = self.env["freight.operation"].browse(docids)
        if not bills:
            raise UserError(_("Current Shipping order have no Bill!!"))
        docargs = {
            "docids": docids,
            "doc_model": "freight.operation",
            "docs": docs,
