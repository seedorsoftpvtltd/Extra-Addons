...
class FreightOperation(models.Model):
    ...
    tracking_ids = fields.One2many(
        "operation.tracking", "operation_id", string="Trackings"
    )
    ...
    exp_send_date = fields.Date(string="Expected Send Date", help="Expected Send Date")
    act_send_date = fields.Date(string="Actual Send Date", help="Actual Send Date")
    exp_rec_date = fields.Date(
        string="Expected Receive Date", help="Expected Received Date"
    )
    act_rec_date = fields.Date(
        string="Actual Receive Date", help="Actual Received Date"
    )
    ...
    act_payment_margin = fields.Float(
        string="Actual Margin", compute="_compute_act_payment_margin"
    )
    inv_amount_due = fields.Float(
        compute="_compute_inv_amount_due", string="Invoice Amount Due"
    )
    bill_amount_due = fields.Float(
        compute="_compute_bill_amount_due", string="Bill Amount Due"
    )
    ...
    total_weight = fields.Float(
        string="Total Weight", store=True, compute="_compute_weight_and_volume"
    )
    total_volume = fields.Float(
        string="Total Volume", store=True, compute="_compute_weight_and_volume"
    )
    bill_count = fields.Integer(compute="_compute_count_bill", string="Bill Counts")
    invoice_count = fields.Integer(
        compute="_compute_count_bill", string="Invoice Counts"
    )
    ...
    def _check_date(self):
        for operation in self:
            ...
            if operation.exp_send_date and operation.exp_send_date < order_date:
                ...
                raise UserError(
                    _(
                        "Actual Received date should be greater or"
                        "equals to Order Date !!"
                    )
                )
    ...
    def _compute_actual_recivable_payment(self):
        ...
        for operation in self:
            ...
            total = sum(inv.amount_total - inv.amount_residual for inv in invs)
    ...
    def _compute_actual_payable_payment(self):
        ...
        for operation in self:
            ...
            total = (
                sum(bill.amount_total - bill.amount_residual for bill in bills) or 0.0
            )
...
#            raise UserError(_("Add at least one order Line !!"))
#        return rec
    ...
    def unlink(self):
        ...
        #super(FreightOperation, self).unlink()
        for operation in self:
            if operation.state not in ["draft", "cancel"]:
                raise UserError(
                    _(
                        "You can't delete this Shipping Operation.\n"
                            ...
                            "invoice_line_ids": [
                                (
                                    ...
                                    0,
                                    {
                                        ...
                                        'tax_ids': [(6, 0, line.x_tax_ids.ids)],
                            },
                                )
                            ]
                        }
...
class FreightOperationLine(models.Model):
    ...
    container_id = fields.Many2one("freight.container", string="Container_Type")
    ...
    sale_price = fields.Float(
        string="Sale Price", compute="_compute_calculate_sale_price", store=True
    )
    price = fields.Float(string="Price")
    exp_gross_weight = fields.Float(
        string="Gross Weight", help="Expected Weight in kg."
    )
    exp_vol = fields.Float(string="Volume", help="Expected Volume in m3 Measure")
    ...
    def _check_weight_volume(self):
        for freight_line in self:
            if freight_line.exp_gross_weight < 0.0:
                raise UserError(_("You can't enter weight in Negative value!!"))
            if freight_line.exp_vol < 0.0:
                raise UserError(_("You can't enter volume in Negative value!!"))
    ...
    def _onchange_container_id(self):
        for freight_line in self:
            if (
                ...
                raise UserError(
                    _("%s is not avilable!!") % (freight_line.container_id.name)
                )"""
...
#                    line.exp_vol = 0.0
    ...
    @api.onchange("volume_price", "weight_price")
    def _onchange_pricee(self):
        for line in self:
            if line.price_list_id:
                if line.billing_on == "volume":
#                    line.exp_gross_weight = 0.0
...
class OperationRoute(models.Model):
    ...
    dest_location = fields.Many2one("freight.port", string="Destination Location")
    transport = fields.Selection(
        [("land", "Land"), ("ocean", "Ocean"), ("air", "Air")], string="Transport"
    )
    operation_id = fields.Many2one("freight.operation", string="Operation")
    service_ids = fields.One2many("operation.service", "route_id", string="Services")
    cost_total = fields.Float(string="Cost", compute="_compute_cost_total", store=True)
    sale_total = fields.Float(string="Sale", compute="_compute_sale_total", store=True)
