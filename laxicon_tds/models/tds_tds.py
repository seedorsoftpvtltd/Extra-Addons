...
class AccountTDS(models.Model):
    _name = 'account.tds.tds'
    _description = "Account TDS Setup"
    ...
    name = fields.Char(string="Section", required=True, copy=False)
    nature = fields.Char(string="Payment Nature", required=True)
    ...
    #  for TDS
    ind_huf_tds_per = fields.Float(string="TDS %")
    other_tds_per = fields.Float(string="Other TDS %")
    ...
    @api.constrains('other_tds_per', 'ind_huf_tds_per')
    def _check_tds_per(self):
        if self.ind_huf_tds_per and self.ind_huf_tds_per > 100 or self.ind_huf_tds_per < 0:
            raise ValidationError('You cannot enter Ind/Huf TDS percentage value greater than 100.')
        if self.other_tds_per and self.other_tds_per > 100 or self.other_tds_per < 0:
            raise ValidationError('You cannot enter Other TDS percentage value greater than 100.')
