...
class KsCurvedBackendTheme(http.Controller):
    ...
    @route(['/ks_curved_theme/hbcompensation'], type='json', auth='user')
    def hbcompensation(self, hb_icons):
        domain = [('compensation', '=', True)]
        menu_ids = request.env['ir.ui.menu'].search(domain)
        icon = []
        for menu in menu_ids:
            for icons in hb_icons:
    ...
    @route(['/ks_curved_theme/hbworkforce'], type='json', auth='user')
    def hbworkforce(self, hb_icons):
        domain = [('workforce', '=', True)]
        menu_ids = request.env['ir.ui.menu'].search(domain)
        icon = []
        for menu in menu_ids:
            for icons in hb_icons:
    ...
    def update_bookmarks(self, **post):
        ...
        return request.env['ir.ui.view'].render_template("ks_curved_backend_theme_seedorpeoplev1.ks_bookmark_", values)
    ...
    def render_theme_view_settings(self, ks_setting_scope, ks_rec_id=None):
        ...
        if ks_setting_scope == 'User':
            ...
            values.update({
                ...
                'ks_manager_role': user.has_group('ks_curved_backend_theme_seedorpeoplev1.ks_curved_theme_settings'),
                'ks_global_config_id': request.env.ref('ks_curved_backend_theme_seedorpeoplev1.ks_global_config_single_rec').id,
            ...
            if ks_current_menu_bar_scope == 'Global':
                ks_origin_data = request.env.ref('ks_curved_backend_theme_seedorpeoplev1.ks_global_config_single_rec')
    ...
    def save_theme_settings(self, **post):
        ...
        if post.get('ks_origin_scope') == 'global':
            ks_origin_obj = request.env.ref('ks_curved_backend_theme_seedorpeoplev1.ks_global_config_single_rec')
            ks_splitter = '_global'
        ...
        if 'ks_enterprise_apps' in post['ks_unsaved_setting']:
            request.env.ref('ks_curved_backend_theme_seedorpeoplev1.ks_curved_theme_hide_enterprise_apps').write({
    ...
    def ks_get_website_title(self, **post):
        ks_origin = request.env.ref('ks_curved_backend_theme_seedorpeoplev1.ks_global_config_single_rec')
    ...
    def set_background_default(self, ks_setting_scope, ks_rec_id=None, ks_default_info=None):
        if ks_setting_scope == 'Global' and ks_default_info:
            ks_global_obj = request.env.ref('ks_curved_backend_theme_seedorpeoplev1.ks_global_config_single_rec')
    ...
    @http.route('/ks_curved_backend_theme_seedorpeoplev1/getTheme', type='json', auth="user")
