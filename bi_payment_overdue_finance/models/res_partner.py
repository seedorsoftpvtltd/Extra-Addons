...
class Res_Partner_Fiannce(models.Model):
	...
	def get_amounts_and_date_amount_over(self):
		...
		for partner in self:
			...
			data = tabulate({"Cur.": currency,"Amount": value}, headers = "keys",tablefmt="plain")
		...
		for partner in self:
			...
			if partners_to_email:
				for partner_to_email in partners_to_email:
					mail_template_id = self.env['ir.model.data'].xmlid_to_object('bi_payment_overdue_finance.email_template_account_followup_default_id')
	...
	def do_button_print(self):
		return self.env.ref('bi_payment_overdue_finance.report_customer_overdue_print_finance').report_action(self)
