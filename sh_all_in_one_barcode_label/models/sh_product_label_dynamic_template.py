        ...
        if self:
            for rec in self:
                ...
                if rec.sh_barcode_field_id and not rec.sh_template_line_ids:
                    print("\n\n\ncalled",rec.sh_barcode_field_id,rec.sh_template_line_ids)
                    raise UserError(_(
                        "Please select fields which you have defined in dynamic barcode lines, If you have not added pls add field first in dynamic barcode label lines."))
...
class ShProductLabelDynamicTemplateLine(models.Model):
    ...
    sh_field_id = fields.Many2one('ir.model.fields', ondelete='cascade', domain=[('model_id.model', '=', 'product.product'), (
        'ttype', 'not in', ['many2many', 'many2one_reference', 'one2many', 'reference'])], string='Field')
