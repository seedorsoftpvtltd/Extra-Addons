...
from odoo.addons.niova_invoice_scan.models.scan_provider_paperflow import ScanProviderPaperflow
import logging
import base64
...
class ScannedVoucher(models.Model):
    ...
    raw_voucher_text = fields.Text(string='Raw Voucher Text', default='', readonly=True, help="Unstructured textual content of a voucher. This is useful for performing full text searching in vouchers.")
    ...
    def receive_scanned_vouchers(self, offset=0, search={}, seen='not_seen'):
        content = self.scan_provider.get_conditional_vouchers(offset, search, VOUCHER_RECORD_COUNT, seen)
        if content:
    ...
    def report_as_done(self):
        ...
        status, _, _ = self.scan_provider.set_vouchers_as_seen(voucher_ids)
        if not status:
    ...
    def _get_raw_voucher_text(self, voucher_id):
        return self.scan_provider.get_voucher_text(voucher_id) if voucher_id else ''

    def _attach_pdf(self):
        self.ensure_one()
        pdf = self.scan_provider.get_voucher_pdf(self.voucher_id)
        if pdf:
    ...
    def scan_provider(self):
        if self._scan_provider is None:
            ...
            client_secrect = settings.get_param('invoice_scan_client_secret', '')
            type(self)._scan_provider = ScanProviderPaperflow(client_secrect)
    ...
    def action_refresh_voucher(self):
        if self.state not in NO_REFRESH_STATES and self.voucher_id:
            voucher = self.scan_provider.get_voucher(self.voucher_id)
            if voucher:
    ...
    def action_open_invoice(self):
        if self.invoice_id:
            [action] = self.env.ref('niova_invoice_scan.action_voucher_vendor_bill').read()
            if self.voucher_type == 'in_refund':
                ctx = {'default_type': 'in_refund', 'type': 'in_refund', 'journal_type': 'purchase'}
            else:
