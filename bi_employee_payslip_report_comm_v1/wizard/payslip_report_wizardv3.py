...
class EmpPayslipReport(models.TransientModel):
    ...
    def employee_payslip_xls(self):
        if self.file_type == 'pdf':
            ...
            return self.env.ref('bi_employee_payslip_report_comm_v1.action_report_export_emp_payslip').report_action(
                self,
                data=datas)
            ...
            for payslip in payslip_ids:
                for line in payslip.line_ids:
                    if line.salary_rule_id.name not in category:
                        category.append(line.salary_rule_id.name)
