...
from odoo import _, api, fields, models, SUPERUSER_ID, tools, registry
...
class MailComposer(models.TransientModel):
    ...
    def get_mail_values(self, res_ids):
        ...
        elif not len(results.get(res_id, {}).get('partner_ids', [])) and (self.email_cc or self.cc_recipient_ids):
            results['cc'] = {
                'subject': self.subject,
                'body': self.body or '',
                'parent_id': self.parent_id and self.parent_id.id,
                ...
                for res_id, mail_values in all_mail_values.items():
                    if wizard.composition_mode == 'mass_mail':
                        ...
                        if res_id not in ['to', 'cc']:
                            ActiveModel.browse(res_id).message_notify(**post_params)
                        else:
    ...
    def message_format(self):
        ...
        for obj in res:
            ...
            cc_partners_list = self.env['res.partner'].browse(
                obj.get('cc_recipient_ids', [])).read(['name','email'])
            for item in cc_partners_list:
                cc_partners += item.get('name') or item.get('email') + ', '
            bcc_partners_list = self.env['res.partner'].browse(
                obj.get('bcc_recipient_ids', [])).read(['name','email'])
            for item in bcc_partners_list:
                bcc_partners += item.get('name') or item.get('email') + ', '
