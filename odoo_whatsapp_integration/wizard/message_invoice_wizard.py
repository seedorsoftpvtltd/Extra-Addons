...
class SendMessage(models.TransientModel):
    ...
    def send_custom_message(self):
        if self.message and self.mobile_number:
            ...
            self.chatter_msg_invoice(msg)
            return send_msg
    def chatter_msg_invoice(self,msg):
        id = self.env['account.move'].browse(self.env.context.get('active_id'))
        print("account")

        res = id.message_post(body=msg,
                              subject="Invoice Whatsapp Message")
        return res