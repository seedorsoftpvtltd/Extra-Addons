...
class SendMessage(models.TransientModel):
    _name = 'whatsapp.wizard.inventory'
    ...
    def send_custom_message(self):
        if self.message and self.mobile_number:
            ...
            self.chatter_msg_inventory(msg)
            return send_msg

    def chatter_msg_inventory(self,msg):
        id = self.env['stock.picking'].browse(self.env.context.get('active_id'))
        print("inven")

        res = id.message_post(body=msg,
                              subject="Inventory Whatsapp Message")
        return res