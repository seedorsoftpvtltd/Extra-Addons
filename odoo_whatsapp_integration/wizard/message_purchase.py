...
class SendMessage(models.TransientModel):
    _name = 'whatsapp.wizard.purchase'
    ...
    def send_custom_message(self):
        if self.message and self.mobile_number:
            ...
            self.chatter_msg_purchase(msg)
            return send_msg

    def chatter_msg_purchase(self,msg):
        id = self.env['purchase.order'].browse(self.env.context.get('active_id'))
        print("purchase")

        res = id.message_post(body=msg,
                              subject="Purchase Whatsapp Message")
        return res