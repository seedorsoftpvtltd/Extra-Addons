...
class Http(models.AbstractModel):
    ...
    def session_info(self):
        ...
        if self.env.user.has_group('base.group_user'):
            res['user_branches']= {'current_branch': (user.branch_id.id, user.branch_id.name),'allowed_branch': [(comp.id, comp.name) for comp in user.branch_ids]}
            res['allowed_branch_ids']= user.branch_id.ids
            res['branch_id']= user.branch_id.id if request.session.uid else None
