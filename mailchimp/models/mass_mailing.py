...
class MassMailing(models.Model):
    ...
    def _select_clause(self):
        select_str = """
            SELECT tmp.email_address AS email,
                   ...
                   mm.sent_date AS scheduled,
                   tmp.opened AS opened,
                   tmp.clicked AS clicked,
                   tmp.bounced AS bounced,
                   tmp.status AS state,
                   (now() AT TIME ZONE 'UTC') AS state_update,
    ...
    def _create_or_update_campaign_statsticts(self):
        self._cr.execute("""
                    UPDATE mailing_trace SET email = mc_report.email,
                        ...
                        sent = mc_report.sent,
                        scheduled = mc_report.scheduled,
                        opened = mc_report.opened,
                        clicked = mc_report.clicked,
                        bounced = mc_report.bounced,
                        state = mc_report.state,
                        state_update = mc_report.state_update,
        ...
        self._cr.execute("""
            INSERT INTO mailing_trace (email, model, trace_type, res_id, sent, scheduled, opened, clicked, bounced, state, state_update, mass_mailing_id)
                %s
                %s
                full outer JOIN mailing_trace mt on mt.res_id = mc.id and mt.mass_mailing_id=%d
            ...
            for queue in queues:
                ...
                for email in email_list:
                    ...
                    for activity in activities:
                        ...
                        if action == 'open':
                            opened = account.covert_date(activity.get('timestamp'))
                        elif action == 'click':
                            clicked = account.covert_date(activity.get('timestamp'))
                        elif action == 'bounce':
                            bounced = account.covert_date(activity.get('timestamp'))
                    state = 'sent'
                    if opened or clicked:
                        state = 'opened'
                    elif bounced or state in ['hard', 'soft']:
                        state = 'bounced'
                    email.update({'status': state, 'opened': opened, 'clicked': clicked, 'bounced': bounced})
    ...
    def fetch_email_activity(self):
        for record in self.search([('state', 'not in', ['draft', 'in_queue']), ('mailchimp_id', '!=', False)]):
            ...
            if record.sent_date and record.sent_date >= pre_date:
