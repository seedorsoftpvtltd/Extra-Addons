...
class MailChimpTemplates(models.Model):
    ...
    def export_update_templates_mailchimp(self, account=False):
        for record in self:
            body_html = self.env['mail.thread']._replace_local_links(record.body_html)
            ...
            if record.template_id:
                return True
                # response = record.account_id._send_request('templates/' + record.template_id, prepared_vals,
                #                                            method='PATCH')
            else:
