...
class MailChimpLists(models.Model):
    ...
    remaining_to_export_count = fields.Integer("Remaining to Export", compute='_compute_remaining_to_export_count')

    def _compute_remaining_to_export_count(self):
        for record in self:
            record.remaining_to_export_count = len(self._get_remaining_partner_to_export())
    ...
    def action_view_remaining_to_export_contact(self):
        self.ensure_one()
        remaining_to_export = self._get_remaining_partner_to_export()
        if remaining_to_export:
            [action] = self.env.ref('base.action_partner_form').read()
            action['domain'] = [('id', 'in', remaining_to_export.ids)]
            return action
        return True
    ...
    def update_partner_detail(self, name, email, partner_detail, old_email=False):
        ...
        if partner_id:
            ...
            if partner_detail:
                partner_detail.get('name', False) and partner_detail.pop('name') or ''
                if self.account_id.is_override_name:
                    partner_detail.update({'name': name})
                partner_detail.update(partner_detail)
