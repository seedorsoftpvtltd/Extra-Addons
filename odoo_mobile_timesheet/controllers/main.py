...
#        'timesheets_count': timesheets_count,
            'employee_timesheets_count': timesheets_count,
        })
        return values
    
#    @http.route(['/my/timesheets', '/my/timesheets/page/<int:page>'], type='http', auth="user", website=True)
    @http.route(['/my/employee/timesheets', '/my/employee/timesheets/page/<int:page>'], type='http', auth="user", website=True)
...
#            'timesheets': timesheets,
            'searched_employee_timesheets': timesheets,
#            'page_name': 'timesheets',
            'page_name': 'search_timesheets_page',
            'sortings' : sortings,
            'sortby': sortby,
            'pager': pager,
#            'default_url': '/my/timesheets',
            'default_url': '/my/employee/timesheets',
...
#    @http.route(['/my/timesheet/<int:timesheet>'], type='http', auth="user", website=True)
    @http.route(['/my/employee/timesheet/<int:timesheet>'], type='http', auth="user", website=True)
    def edit_timesheet(self, timesheet=None, **kw):
        ...
        values={
            ...
            'line_date': line_date,
            'work_type_ids':work_type_ids,
            'page_name': 'edit_employee_timesheet_page',
            'portal_timesheet':True,
        }
...
#                url="/my/timesheets",
            ...
            domain += [('date', '=', kw.get('start_date'))]
#            timesheets = timesheets_obj.search(domain, order=order, limit=self._items_per_page, offset=pager['offset'])
            timesheets = timesheets_obj.sudo().search(domain, order=order, offset=pager['offset'])
            values.update({
#                'timesheets': timesheets,
                'employee_timesheets': timesheets,
#                'page_name': 'timesheets',
                'page_name': 'employee_timesheets_page',
