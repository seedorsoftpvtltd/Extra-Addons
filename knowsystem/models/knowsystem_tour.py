...
class knowsystem_tour(models.Model):
    ...
    def action_keep_the_tour(self):
        ...
        """
        self.ensure_one()
        action_id = self.env.ref("knowsystem.knowsystem_tour_action_form_article")
    ...
    def return_form_view(self):
         ...
         * action
        """
        action_id = self.env.ref("knowsystem.knowsystem_tour_action_form_only")
        action = action_id.read()[0]
        action["res_id"] = self.id
        return action
    ...
    def return_popup_form_view(self):
         ...
         * action
        """
        view_id = self.env.ref("knowsystem.knowsystem_tour_view_form").id
        return view_id
    ...
    def return_start_page(self):
         ...
         * action
        """
        action_id = self.env.ref("knowsystem.knowsystem_tour_action_form_start")
        action = action_id.read()[0]
        action["res_id"] = self.id
        return action
