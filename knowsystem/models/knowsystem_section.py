...
class knowsystem_section(models.Model):
    ...
    def return_edit_form(self):
        ...
        """
        view_id = self.env.ref('knowsystem.knowsystem_section_view_form').id
        return view_id
    ...
    def return_article_search(self, res_model, res_id):
        """
        The method to return articles search view
        ...
        """
        view_id = self.env.ref('knowsystem.article_search_form_view').id
        context = self.env.context.copy()
        tags = self.env["knowsystem.tag"]._return_tags_for_document(res_model=res_model, res_id=res_id)
        context.update({"default_tag_ids": tags.ids,})
        res = {
            "view_id": view_id,
            "context": context,
        }
