...
class knowsystem_article(models.Model):
    ...
    def _compute_internal_url(self):
        ...
        for article in self:
            ...
            action_id = self.env.ref("knowsystem.knowsystem_article_action").id
            menu_id = self.env.ref("knowsystem.menu_knowsystem_articles").id
    ...
    def _notify_of_revisions(self):
        ...
        for article in self:
            ...
            article.message_post(
                body=body_html,
                subject=subject,
                subtype="knowsystem.mt_knowsystem_revisions",
            )
    ...
    def select_template(self):
        ...
        """
        action_id = self.env.ref("knowsystem.create_from_template_action")
    ...
    def get_info_formview_id(self, access_uid=None):
        ...
        """
        view_id = self.env.ref('knowsystem.knowsystem_article_view_form_info').id
        return view_id
    ...
    def get_rights_formview_id(self, access_uid=None):
        ...
        """
        view_id = self.env.ref('knowsystem.knowsystem_article_view_form_rights').id
        return view_id
    ...
    def return_mass_update_wizard(self):
        ...
        """
        view_id = self.env.ref('knowsystem.article_update_form_view').id
        return view_id
    ...
    def return_add_to_tour_wizard(self):
        ...
        """
        view_id = self.env.ref('knowsystem.add_to_tour_form_view').id
        return view_id
    ...
    def action_make_template(self):
        ...
        """
        self.ensure_one()
        action_id = self.env.ref("knowsystem.knowsystem_article_template_action_only_form")
        action = action_id.read()[0]
    ...
    def proceed_article_action(self, action):
        ...
        if action == "share_url":
            ...
            report = self.env.ref('knowsystem.action_report_knowsystem_article')
            attachment_ids = []
            for article in self:
                result, report_format = report.render_qweb_pdf(article.ids)
