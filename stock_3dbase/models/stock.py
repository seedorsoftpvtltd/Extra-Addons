...
class StockLocationTag(models.Model):
    ...
    opacity = fields.Integer(string='Opacity', default=500)


class StockWarehouse(models.Model):
    _inherit = 'stock.warehouse'

    spacex = fields.Integer('Depth (X)', default=0)
    spacey = fields.Integer('Width (Y)', default=0)
    spacez = fields.Integer('Height (Z)', default=0)
    planimetry_image = fields.Binary('Planimetry Image', help="Warehouse planimetry image")

    count_sl = fields.Integer('Count Stock Locations', compute='_compute_count_sl')
