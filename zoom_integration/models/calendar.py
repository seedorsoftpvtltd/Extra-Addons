...
class Meeting(models.Model):
    ...
    def _compute_todays_calendar_events(self):
        ...
        for rec in self:
            rec.todays_meeting = ''
            if not rec.create_uid or (rec.create_uid and rec.create_uid.id == self.env.user.id):
                if not rec.recurrency and rec.start_datetime and rec.is_zoom_meeting:
                    local_date_time = rec._datetime_localize(rec.start_datetime)
                    ...
                    todays_meeting = self.env['calendar.event'].search(
                        [('id', '!=', rec.id), ('is_zoom_meeting', '=', True), ('create_uid', '=', self.env.user.id),
                    ...
                    if todays_meeting:
                        ...
                        for today_meeting in todays_meeting:
                            meeting_date = rec._datetime_localize_user_format(today_meeting.start_datetime)
                            description = ''
                        ...
                        rec.todays_meeting = todays_meeting_description
                    else:
                        rec.todays_meeting = ''
                else:
                    rec.todays_meeting = ''
...
#               rec.todays_meeting = ''
    ...
    def get_visibility_check(self):
        for record in self:
            ...
            if record.is_zoom_meeting !=True and record.id != False :
