            ...
            for rec in self:
                if rec.company_id.sh_free_product_tax == 'yes':
                    if rec.sh_free_product == True:
                        ...
                        if rec.invoice_line_ids:
                            for line in rec.invoice_line_ids:
                                line.price_unit = line.sh_unit_price
    
    @api.onchange('invoice_line_ids.sh_unit_price','line_ids', 'invoice_payment_term_id', 'invoice_date_due', 'invoice_cash_rounding_id', 'invoice_vendor_bill_id')
        ...
        def _compute_base_line_taxes(base_line):
            ...
            if move.is_invoice(include_receipts=True):
                ...
                if base_line.currency_id:
                    if move.company_id.sh_free_product_tax == 'yes':
                        if move.sh_free_product == True:
                            move.amount_total = 0.0
                            move.amount_untaxed = 0.0
                            price_unit_foreign_curr = sign * base_line.sh_unit_price * (1 - (base_line.discount / 100.0))
                    ...
                    if move.company_id.sh_free_product_tax == 'yes':
                        if move.sh_free_product == True:
                            ...
                            price_unit_comp_curr = sign * base_line.sh_unit_price * (1 - (base_line.discount / 100.0))
                        else:
                            price_unit_foreign_curr = 0.0
...
class AccountMoveLine(models.Model):
    ...
    def _onchange_product_id(self):
        for line in self:
            ...
            line.sh_unit_price = line._get_computed_price_unit()
            ...
            if line.tax_ids and line.move_id.fiscal_position_id:
                ...
                line.sh_unit_price = line._get_fields_onchange_balance(balance=balance).get('sh_unit_price', line.sh_unit_price)
            ...
            line.sh_unit_price = company.currency_id._convert(line.sh_unit_price, line.move_id.currency_id, company, line.move_id.date)
