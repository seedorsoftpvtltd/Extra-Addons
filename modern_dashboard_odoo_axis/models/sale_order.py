    ...
    def get_sale_table(self, option):
        if option == 'today_sales':
            sql_query = """
                ...
                ORDER BY order_reference limit 10
            """
    ...
    def get_quotation_table(self,option):
        ...
        if option == 'monthly_sales':
            sql_query = """
               ...
               so.commitment_date AS delievery_date
                FROM sale_order so INNER JOIN res_partner rs ON (so.partner_id = rs.id)
                WHERE EXTRACT(month FROM so.date_order::date) = EXTRACT(month FROM CURRENT_DATE) AND so.state = 'sent' limit 10 
        ...
        if option == 'year_sales':
            sql_query = """
               ...
               so.commitment_date AS delievery_date
                FROM sale_order so INNER JOIN res_partner rs ON (so.partner_id = rs.id)
                WHERE EXTRACT(year FROM so.date_order::date) = EXTRACT(year FROM CURRENT_DATE) AND so.state = 'sent'  limit 10 
    ...
    def get_cancel_sale_table(self,option):
        if option == 'today_sales': 
            sql_query = """
                SELECT so.name AS order_reference,rs.name AS partner_name,so.date_order AS date_order
                FROM sale_order so INNER JOIN res_partner rs ON (so.partner_id = rs.id)
                WHERE so.date_order >= current_date AND so.state = 'cancel' limit 10 
        ...
        if option == 'monthly_sales':
            sql_query = """
               SELECT so.name AS order_reference,rs.name AS partner_name,so.date_order AS date_order
                FROM sale_order so INNER JOIN res_partner rs ON (so.partner_id = rs.id)
                WHERE EXTRACT(month FROM so.date_order::date) = EXTRACT(month FROM CURRENT_DATE) AND so.state = 'cancel' limit 10  
        ...
        if option == 'year_sales':
            sql_query = """
               SELECT so.name AS order_reference,rs.name AS partner_name,so.date_order AS date_order
                FROM sale_order so INNER JOIN res_partner rs ON (so.partner_id = rs.id)
                WHERE EXTRACT(year FROM so.date_order::date) = EXTRACT(year FROM CURRENT_DATE) AND so.state = 'cancel'  limit 10
        ...
        if option == 'all':
            sql_query = """
               SELECT so.name AS order_reference,rs.name AS partner_name,so.date_order AS date_order
                FROM sale_order so INNER JOIN res_partner rs ON (so.partner_id = rs.id)
                WHERE so.state = 'cancel' limit 10 
