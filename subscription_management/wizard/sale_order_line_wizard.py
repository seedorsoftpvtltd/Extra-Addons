...
class AddProduct(models.TransientModel):
   ...
   # @api.multi
    def create_sale_order_line(self):
        ...
        line_id = self.env['sale.order.line'].create({'product_id':self.product_id.id,'order_id':sale_order.id,'product_uom':self.product_id.uom_id.id,'name':name, 'price_unit':self.product_id.list_price,'tax_id':[(6, 0, self.product_id.taxes_id.ids)]})
        if sale_order.pricelist_id and sale_order.partner_id:
            line_id.price_unit = self.env['account.tax']._fix_tax_included_price_company(line_id._get_display_price(product), product.taxes_id, line_id.tax_id, line_id.company_id)
        return line_id
