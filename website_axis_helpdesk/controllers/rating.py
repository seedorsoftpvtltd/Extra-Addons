...
class WebsiteHelpdesk(http.Controller):
    ...
    def index(self, **kw):
        ...
        values = {'teams': teams}
        return request.render('website_axis_helpdesk.index', values)
    ...
    def page(self, team, project_id=None, **kw):
        ...
        if not (team.use_rating and team.portal_show_rating) and not user.has_group('website_axis_helpdesk.group_helpdesk_manager'):
            raise NotFound()
        ...
        return request.render('website_axis_helpdesk.team_rating_page', values)
