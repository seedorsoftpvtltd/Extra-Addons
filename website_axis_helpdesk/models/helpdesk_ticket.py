...
class HelpdeskSLAStatus(models.Model):
    ...
    def _compute_status(self):
        ...
        for status in self:
            ...
            # deadline will be set at time I don't work (ticket creation time might not be in working calendar).
            status.deadline = working_calendar.plan_hours(status.sla_id.time_hours, deadline, compute_leaves=True)
            if status.reached_datetime:  # if reached_datetime, SLA is finished: either failed or succeeded
                ...
                status.status = 'ongoing' if status.deadline > fields.Datetime.now() else 'failed'
...
class HelpdeskTicket(models.Model):
    ...
    user_id = fields.Many2one('res.users', string='Assigned to', tracking=True, domain=lambda self: [('groups_id', 'in', self.env.ref('website_axis_helpdesk.group_helpdesk_user').id)])
    ...
    def create(self, list_value):
        ...
        # to avoid intrusive changes in the 'mail' 
        ...
        template = self.env.ref('website_axis_helpdesk.ticket_template')
    ...
    def _creation_subtype(self):
        return self.env.ref('website_axis_helpdesk.mt_ticket_new')

    def _track_subtype(self, init_values):
        self.ensure_one()
        if 'stage_id' in init_values:
            return self.env.ref('website_axis_helpdesk.mt_ticket_stage')
    ...
    def _notify_get_groups(self):
        ...
        helpdesk_user_group_id = self.env.ref('website_axis_helpdesk.group_helpdesk_user').id
        new_groups = [(
            'group_helpdesk_user',
    ...
    def rating_apply(self, rate, token=None, feedback=None, subtype=None):
        return super(HelpdeskTicket, self).rating_apply(rate, token=token, feedback=feedback, subtype="website_axis_helpdesk.mt_ticket_rated")
