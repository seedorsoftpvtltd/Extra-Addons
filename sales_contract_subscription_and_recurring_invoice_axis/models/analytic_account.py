...
#                       assert isinstance(record.end_contract_date, datetime)
    ...
    def _find_mail_template(self, force_confirmation_template=False):
        ...
        if force_confirmation_template or (self.state != 'locked'):
            template_id = int(self.env['ir.config_parameter'].sudo().get_param('sales_contract_subscription_and_recurring_invoice_axis.default_template'))
            template_id = self.env['mail.template'].search([('id', '=', template_id)]).id
            if not template_id:
                template_id = self.env['ir.model.data'].xmlid_to_res_id('sales_contract_subscription_and_recurring_invoice_axis.mail_template_contract', raise_if_not_found=False)
        return template_id
