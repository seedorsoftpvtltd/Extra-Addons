...
class TestReorderingRule(TransactionCase):
    ...
    def test_reordering_rule(self):
        ...
        warehouse_1.write({'reception_steps': 'tpo_steps'})
        ...
        with product_form.seller_ids.new() as seller:
            seller.name = partner
        product_form.route_ids.add(self.env.ref('warehouse_stock.route_warehouse0_buy'))
        ...
        # Check warehouse order created or not
        ...
        # On the po generated, the source document should be the name of the reordering rule
        self.assertEqual(order_point.name, warehouse_order.origin, 'Source document on warehouse order should be the name of the reordering rule.')
    ...
    def test_procure_not_default_partner(self):
        ...
        used."""
        warehouse_route = self.env.ref("warehouse_stock.route_warehouse0_buy")
        ...
        product = self.env["product.product"].create({
            ...
            "uom_id": uom_unit.id,
            "default_code": "A",
            "route_ids": [(6, 0, warehouse_route.ids)],
        })
