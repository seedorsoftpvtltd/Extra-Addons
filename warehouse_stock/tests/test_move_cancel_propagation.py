...
class TestMoveCancelPropagation(Testwarehouse):
    ...
    def test_01_cancel_draft_warehouse_order_one_steps(self):
        """ Check the picking and moves status related PO, When canceling warehouse order
            Ex.
                1) Set one steps of receiption and delivery on the warehouse.
                2) Create Delivery order with mto move and confirm the order, related Booking should be generated.
                3) Cancel 'draft' warehouse order should not cancel < Delivery >
        """
        ...
        warehouse_order = self.env['warehouse.order'].search([('partner_id', '=', self.customer.id)])

        # Po should be create related picking.
        self.assertTrue(warehouse_order, 'No warehouse order created.')

        # Check status of warehouse Order
        self.assertEqual(warehouse_order.state, 'draft', "warehouse order should be in 'draft' state.")
    ...
    def test_02_cancel_confirm_warehouse_order_one_steps(self):
        """ Check the picking and moves status related warehouse order, When canceling warehouse order
            after confirming.
            Ex.
                1) Set one steps of receiption and delivery on the warehouse.
                2) Create Delivery order with mto move and confirm the order, related Booking should be generated.
                3) Cancel 'confirmed' warehouse order, should cancel releted < Receiption >
        ...
        picking_in = warehouse_order.picking_ids.filtered(lambda r: r.picking_type_id == self.warehouse.in_type_id)
        # Cancel warehouse order.
        warehouse_order .button_cancel()
    ...
    def test_03_cancel_draft_warehouse_order_tpo_steps(self):
        """ Check the picking and moves status related PO, When canceling warehouse order
            in 'draft' state.
            Ex.
                1) Set two steps of receiption and delivery on the warehouse.
                2) Create Delivery order with mto move and confirm the order, related Booking should be generated.
                3) Cancel 'draft' warehouse order should cancel < Input to Stock>
                  but it should not cancel < PICK, Delivery >
        """
        self.warehouse.write({'delivery_steps': 'pick_ship', 'reception_steps': 'tpo_steps'})
        self.picking_out.write({'location_id': self.warehouse.out_type_id.default_location_src_id.id})
        self.move.write({'location_id': self.warehouse.out_type_id.default_location_src_id.id})
        self.picking_out.action_confirm()

        # Find warehouse order related to picking.
        warehouse_order = self.env['warehouse.order'].search([('partner_id', '=', self.customer.id)])
        # warehouse order should be created for picking.
        self.assertTrue(warehouse_order, 'No warehouse order created.')
    ...
    def test_04_cancel_confirm_warehouse_order_tpo_steps(self):
        """ Check the picking and moves status related PO, When canceling warehouse order
            Ex.
                1) Set 2 steps of receiption and delivery on the warehouse.
                2) Create Delivery order with mto move and confirm the order, related Booking should be generated.
                3) Cancel 'comfirm' warehouse order should cancel releted < Receiption Picking IN, INT>
                  not < PICK, SHIP >
        """
        self.warehouse.write({'delivery_steps': 'pick_ship', 'reception_steps': 'tpo_steps'})
        self.picking_out.write({'location_id': self.warehouse.out_type_id.default_location_src_id.id})
        ...
        warehouse_order = self.env['warehouse.order'].search([('partner_id', '=', self.customer.id)])
        # Po should be create related picking.
        self.assertTrue(warehouse_order, 'warehouse order is created.')
    ...
    def test_05_cancel_draft_warehouse_order_three_steps(self):
        """ Check the picking and moves status related PO, When canceling warehouse order
            Ex.
                1) Set 3 steps of receiption and delivery on the warehouse.
                2) Create Delivery order with mto move and confirm the order, related Booking should be generated.
                3) Cancel 'draft' warehouse order should cancel releted < Receiption Picking  IN>
                  not < PICK, PACK, SHIP >
        """
    ...
    def test_06_cancel_confirm_warehouse_order_three_steps(self):
        """ Check the picking and moves status related PO, When canceling warehouse order
            Ex.
                1) Set 3 steps of receiption and delivery on the warehouse.
                2) Create Delivery order with mto move and confirm the order, related Booking should be generated.
    ...
    def test_cancel_move_lines_operation(self):
        """Check for done and cancelled moves. Ensure that the Booking cancellation
