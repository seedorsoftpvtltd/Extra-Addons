...
class TestStockValuation(TransactionCase):
    ...
    def test_change_unit_cost_average_1(self):
        """ Confirm a warehouse order and create the associated receipt, change the unit cost of the
        warehouse order before validating the receipt, the value of the received goods should be set
        ...
        # the unit price of the warehouse order line is copied to the in move
        self.assertEquals(move1.price_unit, 100)

        # update the unit price on the warehouse order line
    ...
    def test_standard_price_change_1(self):
        """ Confirm a warehouse order and create the associated receipt, change the unit cost of the
        warehouse order and the standard price of the product before validating the receipt, the
        ...
        # the move's unit price reflects the warehouse order line's cost even if it's useless when
    ...
    def test_change_currency_rate_average_1(self):
        """ Confirm a warehouse order in another currency and create the associated receipt, change
        ...
        # default currency is USD, create a warehouse order in EUR
        po1 = self.env['warehouse.order'].create({
            'partner_id': self.partner_id.id,
        ...
        # the unit price of the move is the unit price of the warehouse order line converted in
...
class TestStockValuationWithCOA(AccountingTestCase):
    ...
    def test_average_realtime_anglo_saxon_valuation_multicurrency_same_date(self):
        ...
        self.product1.product_tmpl_id.warehouse_method = 'warehouse'
        ...
        # Proceed
        po = self.env['warehouse.order'].create({
            'currency_id': self.eur_currency.id,
            'partner_id': self.partner_id.id,
            'order_line': [
                (0, 0, {
        ...
        inv = self.env['account.move'].with_context(default_type='in_invoice').create({
            ...
            'invoice_line_ids': [(0, 0, {
                ...
                'price_unit': 100.0,
                'product_id': self.product1.id,
                'warehouse_line_id': po.order_line.id,
                'quantity': 1.0,
        ...
        inv = self.env['account.move'].with_context(default_type='in_invoice').create({
            ...
            'invoice_line_ids': [
                (0, 0, {
                    ...
                    'price_unit': 100.0,
                    'product_id': self.product1.id,
                    'warehouse_line_id': line_product_average.id,
                    'quantity': 1.0,
                    'account_id': self.stock_input_account.id,
                }),
                (0, 0, {
                    ...
                    'price_unit': 70.0,
                    'product_id': product_standard.id,
                    'warehouse_line_id': line_product_standard.id,
                    'quantity': 1.0,
                    'account_id': self.stock_input_account.id,
                })
            ]
    ...
    def test_average_realtime_with_delivery_anglo_saxon_valuation_multicurrency_different_dates(self):
        ...
        (picking.move_lines
            .filtered(lambda l: l.warehouse_line_id == line_product_avg)
        ...
        inv = self.env['account.move'].with_context(default_type='in_invoice').create({
            ...
            'invoice_line_ids': [
                (0, 0, {
                    ...
                    'price_unit': 30.0,
                    'product_id': product_avg.id,
                    'warehouse_line_id': line_product_avg.id,
                    'quantity': 1.0,
                    'account_id': self.stock_input_account.id,
                })
            ]
    ...
    def test_average_realtime_with_tpo_delivery_anglo_saxon_valuation_multicurrency_different_dates(self):
        """
        The PO and invoice are in the same foreign currency.
    ...
    def test_anglosaxon_valuation_price_total_diff_discount(self):
        ...
        invoice_form.warehouse_id = order
