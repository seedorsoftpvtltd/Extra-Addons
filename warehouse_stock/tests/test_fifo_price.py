...
class TestFifoPrice(Testwarehouse):
    ...
    def test_00_test_fifo(self):
        ...
        # I create a draft warehouse Order for first in move for 10 kg at 50 euro
        ...
        # Check the "warehouse" status of warehouse order 1
        self.assertEquals(warehouse_order_1.state, 'warehouse')

        # Process the reception of warehouse order 1 and set date
        ...
        # I create a draft warehouse Order for second shipment for 30 kg at 80 euro
        ...
        warehouse_order_usd = self.env['warehouse.order'].create({
            'partner_id': self.env.ref('base.res_partner_3').id,
        ...
        # Confirm the warehouse order in USD
        warehouse_order_usd.button_confirm()
        # Process the reception of warehouse order with USD
        picking = warehouse_order_usd.picking_ids[0]
        self.env['stock.immediate.transfer'].create({'pick_ids': [(4, picking.id)]}).process()
        ...
        product_fifo_negative.categ_id.property_stock_account_input_categ_id = self.ref('warehouse.o_expense')
        product_fifo_negative.categ_id.property_stock_account_output_categ_id = self.ref('warehouse.o_income')
        ...
        # Receive warehouse order with 50 kg Ice Cream at 50€/kg
        warehouse_order_neg = self.env['warehouse.order'].create({
            'partner_id': self.env.ref('base.res_partner_3').id,
            'order_line': [(0, 0, {
                'name': 'FIFO Ice Cream',
                'product_id': product_fifo_negative.id,
        ...
        # I confirm the first warehouse order
        warehouse_order_neg.button_confirm()

        # Process the reception of warehouse order neg
        picking = warehouse_order_neg.picking_ids[0]
        self.env['stock.immediate.transfer'].create({'pick_ids': [(4, picking.id)]}).process()

        # Receive warehouse order with 600 kg FIFO Ice Cream at 80 euro/kg
        warehouse_order_neg2 = self.env['warehouse.order'].create({
            'partner_id': self.env.ref('base.res_partner_3').id,
            'order_line': [(0, 0, {
                'name': product_cable_management_box.name,
                'product_id': product_fifo_negative.id,
        ...
        # I confirm the second negative warehouse order
        warehouse_order_neg2.button_confirm()

        # Process the reception of warehouse order neg2
        picking = warehouse_order_neg2.picking_ids[0]
        self.env['stock.immediate.transfer'].create({'pick_ids': [(4, picking.id)]}).process()
