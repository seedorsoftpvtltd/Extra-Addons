...
class Testwarehouse(TestStockCommon):
    ...
    def _create_make_procurement(self, product, product_qty, date_planned=False):
        ...
        order_values = {
            ...
            'action': 'pull_push',
            'date_planned': date_planned or fields.Datetime.to_string(fields.datetime.now() + timedelta(days=10)),  # 10 days added to current date of procurement to get future schedule date and order date of warehouse order.
    ...
    def _load(self, module, *args):
        tools.convert_file(self.cr, 'warehouse',
    ...
    def setUpClass(cls):
        ...
        cls.res_users_warehouse_user = cls.env['res.users'].create({
            'company_id': cls.env.ref('base.main_company').id,
            'name': "warehouse User",
            'login': "pu",
            'email': "warehouseuser@yourcompany.com",
            'groups_id': [(6, 0, [cls.env.ref('warehouse.group_warehouse_user').id])],
            })
