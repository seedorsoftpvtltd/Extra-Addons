...
class TestValuationReconciliation(ValuationReconciliationTestCase):
    ...
    def setUp(self):
        ...
        self.price_dif_account = self.env['account.account'].create({
            'name': 'Test price dif',
            'code': 'warehouse_account_TEST_42',
    ...
    def _create_warehouse(self, product, date, quantity=1.0, set_tax=False):
        rslt = self.env['warehouse.order'].create({
    ...
    def _create_invoice_for_po(self, warehouse_order, date):
        ...
        move_form.warehouse_id = warehouse_order
    ...
    def test_shipment_invoice(self):
        ...
        warehouse_order = self._create_warehouse(test_product, date_po_and_delivery)
        self._process_pickings(warehouse_order.picking_ids, date=date_po_and_delivery)

        invoice = self._create_invoice_for_po(warehouse_order, '2018-02-02')
        ...
        invoice.post()
        picking = self.env['stock.picking'].search([('warehouse_id','=',warehouse_order.id)])
    ...
    def test_invoice_shipment(self):
        ...
        warehouse_order = self._create_warehouse(test_product, '2017-12-01')

        invoice = self._create_invoice_for_po(warehouse_order, '2017-12-23')
        ...
        invoice.post()
        self._process_pickings(warehouse_order.picking_ids, date='2017-12-24')
        picking = self.env['stock.picking'].search([('warehouse_id', '=', warehouse_order.id)])
        self.check_reconciliation(invoice, picking)
    ...
    def test_multiple_shipments_invoices(self):
        ...
        warehouse_order = self._create_warehouse(test_product, date_po_and_delivery0, quantity=5.0)
        self._process_pickings(warehouse_order.picking_ids, quantity=2.0, date=date_po_and_delivery0)
        picking = self.env['stock.picking'].search([('warehouse_id', '=', warehouse_order.id)], order="id asc", limit=1)

        invoice = self._create_invoice_for_po(warehouse_order, '2017-01-15')
        move_form = Form(invoice)
        with move_form.invoice_line_ids.edit(0) as line_form:
        ...
        invoice2 = self._create_invoice_for_po(warehouse_order, '2017-02-15')
        ...
        # will be taken
        self._process_pickings(warehouse_order.picking_ids.filtered(lambda x: x.state != 'done'), quantity=3.0)
        picking = self.env['stock.picking'].search([('warehouse_id', '=', warehouse_order.id)], order='id desc', limit=1)
    ...
    def test_rounding_discount(self):
        ...
        warehouse_order = self._create_warehouse(test_product, date_po_and_delivery, quantity=10000, set_tax=True)
        self._process_pickings(warehouse_order.picking_ids, date=date_po_and_delivery)

        invoice = self._create_invoice_for_po(warehouse_order, '2018-01-01')
    ...
    def test_rounding_price_unit(self):
        ...
        warehouse_order = self._create_warehouse(test_product, date_po_and_delivery, quantity=100000)
        self._process_pickings(warehouse_order.picking_ids, date=date_po_and_delivery)
