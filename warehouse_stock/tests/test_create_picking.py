...
class TestCreatePicking(common.TestProductCommon):
    ...
    def setUp(self):
        ...
        res_users_warehouse_user = self.env.ref('warehouse.group_warehouse_user')

        Users = self.env['res.users'].with_context({'no_reset_password': True, 'mail_create_nosubscribe': True})
        self.user_warehouse_user = Users.create({
            'name': 'Pauline Poivraisselle',
            'login': 'pauline',
            'email': 'pur@example.com',
            'notification_type': 'inbox',
            'groups_id': [(6, 0, [res_users_warehouse_user.id])]})
    ...
    def test_00_create_picking(self):
        ...
        # Change warehouse order line product quantity
        ...
        self.assertEqual(self.po.order_line.mapped('qty_received'), [7.0], 'warehouse: all products should be received')
    ...
    def test_01_check_double_validation(self):
        ...
        self.env.company.write({'po_double_validation': 'tpo_step','po_double_validation_amount':2000.00})

        # Draft warehouse order created
        self.po = self.env['warehouse.order'].with_user(self.user_warehouse_user).create(self.po_vals)
        self.assertTrue(self.po, 'warehouse: no warehouse order created')

        # warehouse order confirm
        self.po.button_confirm()
        self.assertEqual(self.po.state, 'to approve', 'warehouse: PO state should be "to approve".')

        # PO approved by manager
        self.po.button_approve()
        self.assertEqual(self.po.state, 'warehouse', 'PO state should be "warehouse".')

    def test_02_check_mto_chain(self):
        """ Simulate a mto chain with a warehouse order. Cancel the
        warehouse order should also change the procure_method of the
        following move to MTS in order to be able to link it to a
        manually created warehouse order.
        """
        ...
        product = self.env['product.product'].create({
            'name': 'product',
            'type': 'product',
            'route_ids': [(4, self.ref('stock.route_warehouse0_mto')), (4, self.ref('warehouse_stock.route_warehouse0_buy'))],
        ...
        warehouse_order = self.env['warehouse.order'].search([('partner_id', '=', partner.id)])
        self.assertTrue(warehouse_order, 'No warehouse order created.')

        # Check warehouse order line data.
        warehouse_order_line = warehouse_order.order_line
        self.assertEqual(warehouse_order_line.product_id, product, 'The product on the warehouse order line is not correct.')
        self.assertEqual(warehouse_order_line.price_unit, seller.price, 'The warehouse order line price should be the same than the seller.')
        self.assertEqual(warehouse_order_line.product_qty, customer_move.product_uom_qty, 'The warehouse order line qty should be the same than the move.')
        self.assertEqual(warehouse_order_line.price_subtotal, 1200.0, 'The warehouse order line subtotal should be equal to the move qty * seller price.')

        warehouse_order.button_cancel()
        self.assertEqual(warehouse_order.state, 'cancel', 'warehouse order should be cancelled.')
        self.assertEqual(customer_move.procure_method, 'make_to_stock', 'Customer move should be passed to mts.')

        warehouse = warehouse_order.create({
        ...
        self.assertTrue(warehouse, 'Booking should be created')
        warehouse.button_confirm()

        picking = warehouse.picking_ids
    ...
    def test_03_uom(self):
        """ Buy a dozen of products stocked in units. Check that the quantities on the warehouse order
        ...
        updated on the moves. Edit the ir.config_parameter to propagate the uom of the warehouse order
        ...
        # buy a dozen
        po = self.env['warehouse.order'].create(self.po_vals)
    ...
    def test_04_mto_multiple_po(self):
        """ Simulate a mto chain with 2 warehouse order.
        Create a move with qty 1, confirm the Booking then create a new
        move that will not be merged in the first one(simulate an increase
        order quantity on a SO). It should generate a new Booking, validate
        ...
        warehouse_order_2 = self.env['warehouse.order'].search([('partner_id', '=', partner.id), ('state', '=', 'draft')])
        self.assertTrue(warehouse_order_2, 'No warehouse order created.')

        warehouse_order_2.button_confirm()

        warehouse_order.picking_ids.move_lines.quantity_done = 80.0
        warehouse_order.picking_ids.button_validate()

        warehouse_order_2.picking_ids.move_lines.quantity_done = 20.0
        warehouse_order_2.picking_ids.button_validate()
    ...
    def create_delivery_order(self, propagate_date, propagate_date_minimum_delta):
        ...
        return delivery_order, warehouse_order
    ...
    def test_05_if_propagate_date(self):
        ...
        delivery_order, warehouse_order = self.create_delivery_order(True, 5)
        ...
        warehouse_order_line.write({'date_planned': warehouse_order_line.date_planned + timedelta(days=5)})

        # Now check scheduled date of delivery order is changed or not.
        self.assertEquals(warehouse_order_line.date_planned, delivery_order.scheduled_date,
    ...
    def test_06_no_propagate_date(self):
        ...
        delivery_order, warehouse_order = self.create_delivery_order(False, 5)
        ...
        # Now check scheduled date of delivery order is changed or not.
        self.assertNotEquals(warehouse_order_line.date_planned, delivery_order.scheduled_date,
    ...
    def test_06_differed_schedule_date(self):
        ...
        po_form = Form(self.env['warehouse.order'])
