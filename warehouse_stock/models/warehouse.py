...
from odoo.addons.warehouse.models.warehouse import warehouseOrder as warehouse
...
class warehouseOrder(models.Model):
    ...
    picking_type_id = fields.Many2one('stock.picking.type', 'Deliver To', states=warehouse.READONLY_STATES, required=True, default=_default_picking_type, domain="['|', ('warehouse_id', '=', False), ('warehouse_id.company_id', '=', company_id)]",
    ...
    def write(self, vals):
        if vals.get('order_line') and self.state == 'warehouse':
            for order in self:
    ...
    def button_approve(self, force=False):
        result = super(warehouseOrder, self).button_approve(force=force)
    ...
    def button_cancel(self):
        for order in self:
            for move in order.order_line.mapped('move_ids'):
                if move.state == 'done':
                    raise UserError(_('Unable to cancel warehouse order %s as some receptions have already been done.') % (order.name))
            # If the product is MTO, change the procure_method of the the closest move to warehouse to MTS.
            # The purpose is to link the po that the user will manually generate to the existing moves's chain.
            if order.state in ('draft', 'sent', 'to approve', 'warehouse'):
                for order_line in order.order_line:
        ...
        return super(warehouseOrder, self).button_cancel()

    def action_view_picking(self):
        """ This function returns an action that display existing picking orders of given warehouse order ids. When only one found, show the picking immediately.
        """
    ...
    def _log_decrease_ordered_quantity(self, warehouse_order_lines_quantities):
        ...
        def _render_note_exception_quantity_po(order_exceptions):
            order_line_ids = self.env['warehouse.order.line'].browse([order_line.id for order in order_exceptions.values() for order_line in order[0]])
            warehouse_order_ids = order_line_ids.mapped('order_id')
            ...
            values = {
                'warehouse_order_ids': warehouse_order_ids,
                'order_exceptions': order_exceptions.values(),
                'impacted_pickings': impacted_pickings,
            }
            return self.env.ref('warehouse_stock.exception_on_po').render(values=values)

        documents = self.env['stock.picking']._log_activity_get_documents(warehouse_order_lines_quantities, 'move_ids', 'DOWN', _keys_in_sorted, _keys_in_groupby)
        filtered_documents = {}
...
class warehouseOrderLine(models.Model):
    ...
    move_ids = fields.One2many('stock.move', 'warehouse_line_id', string='Reservation', readonly=True, ondelete='set null', copy=False)
    orderpoint_id = fields.Many2one('stock.warehouse.orderpoint', 'Orderpoint')
    move_dest_ids = fields.One2many('stock.move', 'created_warehouse_line_id', 'Downstream Moves')
    ...
    def _compute_qty_received_method(self):
        super(warehouseOrderLine, self)._compute_qty_received_method()
    ...
    def _compute_qty_received(self):
        super(warehouseOrderLine, self)._compute_qty_received()
        for line in self:
    ...
    def write(self, values):
        ...
        result = super(warehouseOrderLine, self).write(values)
        if 'product_qty' in values:
            self.filtered(lambda l: l.order_id.state == 'warehouse')._create_or_update_picking()
        return result
    ...
    def _create_or_update_picking(self):
        for line in self:
            if line.product_id and line.product_id.type in ('product', 'consu'):
                ...
                #         'note': _('The quantities on your warehouse order indicate less than billed. You should ask for a refund. '),
    ...
    def _find_candidate(self, product_id, product_qty, product_uom, location_id, name, origin, company_id, values):
        ...
        return lines and lines[0] or self.env['warehouse.order.line']
