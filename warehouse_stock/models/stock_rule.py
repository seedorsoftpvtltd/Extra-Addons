...
class StockRule(models.Model):
    ...
    def _run_buy(self, procurements):
        ...
        for procurement, rule in procurements:
            ...
            if not supplier:
                msg = _('There is no matching vendor price to generate the warehouse order for product %s (no vendor defined, minimum quantity not reached, dates not valid, ...). Go on the product form and complete the list of vendors.') % (procurement.product_id.display_name)
                raise UserError(msg)
        ...
        for domain, procurements_rules in procurements_by_po_domain.items():
            ...
            po = self.env['warehouse.order'].sudo().search([dom for dom in domain], limit=1)
            company_id = procurements[0].company_id
            if not po:
                # We need a rule to generate the PO. However the rule generated
                # the same domain for PO and the _prepare_warehouse_order method
                # should only uses the common rules's fields.
                vals = rules[0]._prepare_warehouse_order(company_id, origins, [p.values for p in procurements])
                # The company_id is the same for all procurements since
                # _make_po_get_domain add the company in the domain.
                # We use SUPERUSER_ID since we don't want the current user to be follower of the PO.
                # Indeed, the current user may be a user without access to warehouse, or even be a portal user.
                po = self.env['warehouse.order'].with_context(force_company=company_id.id).with_user(SUPERUSER_ID).create(vals)
            else:
                # If a warehouse order is found, adapt its `origin` field.
                if po.origin:
            ...
            for product, po_lines in grouped_po_lines:
                po_lines_by_product[product] = self.env['warehouse.order.line'].concat(*list(po_lines))
            po_line_values = []
            for procurement in procurements:
                po_lines = po_lines_by_product.get(procurement.product_id.id, self.env['warehouse.order.line'])
                po_line = po_lines._find_candidate(*procurement)

                if po_line:
                    # If the procurement can be merge in an existing line. Directly
                    # write the new values on it.
                    vals = self._update_warehouse_order_line(procurement.product_id,
                    ...
                    po_line_values.append(self._prepare_warehouse_order_line(
                        procurement.product_id, procurement.product_qty,
                        procurement.product_uom, procurement.company_id,
                        procurement.values, po))
            self.env['warehouse.order.line'].sudo().create(po_line_values)
    ...
    def _get_procurements_to_merge(self, procurements):
        ...
        that would use the same warehouse order line.
    ...
    def _update_warehouse_order_line(self, product_id, product_qty, product_uom, company_id, values, line):
    ...
    @api.model
    def _prepare_warehouse_order_line(self, product_id, product_qty, product_uom, company_id, values, po):
        partner = values['supplier'].name
        procurement_uom_po_qty = product_uom._compute_quantity(product_qty, product_id.uom_po_id)
        ...
        if product_lang.description_purchase:
            name += '\n' + product_lang.description_warehouse

        date_planned = self.env['warehouse.order.line']._get_date_planned(seller, po=po)
    ...
    def _prepare_warehouse_order(self, company_id, origins, values):
        """ Create a warehouse order for procuremets that share the same domain
        ...
        values = values[0]
        partner = values['supplier'].name
        warehouse_date = schedule_date - relativedelta(days=supplier_delay)
        ...
        return {
            ...
            'currency_id': partner.with_context(force_company=company_id.id).property_warehouse_currency_id.id or company_id.currency_id.id,
