...
class AccountMove(models.Model):
    ...
    def _stock_account_prepare_anglo_saxon_in_lines_vals(self):
        ...
        for move in self:
            ...
            for line in move.invoice_line_ids.filtered(lambda line: line.product_id.type == 'product' and line.product_id.valuation == 'real_time'):
                ...
                if line.product_id.cost_method != 'standard' and line.warehouse_line_id:
                    po_currency = line.warehouse_line_id.currency_id
                    po_company = line.warehouse_line_id.company_id

                    # Retrieve stock valuation moves.
                    valuation_stock_moves = self.env['stock.move'].search([
                        ('warehouse_line_id', '=', line.warehouse_line_id.id),
                        ('state', '=', 'done'),
                        ('product_qty', '!=', 0.0),
                    ])
                    ...
                    if valuation_stock_moves:
                        ...
                        valuation_price_unit = po_currency._convert(
                            line.warehouse_line_id.price_unit, move.currency_id,
                            po_company, move.date, round=False,
                        )
                    else:
                        # For average/fifo/lifo costing method, fetch real cost price from incoming moves.
                        price_unit = line.warehouse_line_id.product_uom._compute_price(line.warehouse_line_id.price_unit, line.product_uom_id)
                        valuation_price_unit = po_currency._convert(
    ...
    def _stock_account_get_last_step_stock_moves(self):
        ...
        for invoice in self.filtered(lambda x: x.type == 'in_invoice'):
            rslt += invoice.mapped('invoice_line_ids.warehouse_line_id.move_ids').filtered(lambda x: x.state == 'done' and x.location_id.usage == 'supplier')
        for invoice in self.filtered(lambda x: x.type == 'in_refund'):
            rslt += invoice.mapped('invoice_line_ids.warehouse_line_id.move_ids').filtered(lambda x: x.state == 'done' and x.location_dest_id.usage == 'supplier')
        return rslt
