$(document).on('load', '#youtube-player1', function(){
    //console.log('Youtube frame loaded');
});

$(document).on('afterLayout', '#youtube-player1', function(){
    //console.log('Works!');
});
...
jQuery(document).ready(function () {
    checkIframeLoaded();
    jQuery(document).on('contextmenu', '.external_video', function() {
		console.log('contextmenu');
        return false;
    });
    jQuery(document).on('contextmenu', '.clapprVideo', function() {
        return false;
    });
    jQuery(document).on('contextmenu', '.vimeoVideo', function() {
        return false;
    });
    jQuery(document).on('contextmenu', '.local_video', function() {
        return false;
    });
    jQuery(document).on('contextmenu', '.zoom_meeting', function() {
        return false;
    });
	jQuery(document).on('contextmenu', '.hidecontrols', function() {
        return false;
    });
	jQuery(document).on('contextmenu', '.drivehidecontrols', function() {
        return false;
    });
	jQuery(document).on('contextmenu', '.youtubehidecontrols', function() {
        return false;
    });
	jQuery(document).on('contextmenu', '.hidecontrols_mini', function() {
        return false;
    });
	jQuery(document).on('contextmenu', '.drivehidecontrols_mini', function() {
        return false;
    });
	jQuery(document).on('contextmenu', '.youtubehidecontrols_mini', function() {
        return false;
    });
	jQuery(document).on('contextmenu', 'iframe', function() {
        return false;
    });
});
...
function checkIframeLoaded() {
    ...
    var iframe = $("iframe[id^='youtube-player']");
    ...
    if(iframe.length){
        ...
        for(var i=0; i < iframe.length; i++){
            ...
            el.on('load', function(){
                ...
                var src = el[0].src;
                var n = src.indexOf("?");
                var yt_url = src.substring(0,n).replace("youtube.com","youtube-nocookie.com");
                //console.log(yt_url);
                //el.attr('src',yt_url);
                /*var getYt = $.ajax({crossOrigin:true, url: yt_url});
                $.when(getYt).done(function(result){
                    console.log(result);
                }).fail(function(error){
                    console.log(error);
                });*/
                ...
                //el.get(0).contentWindow.print();
                //var bt = el.contents().find(".ytp-chrome-top-buttons");
                //console.log(bt);
                //bt.css("display", "none");
                //var bt = el.contents();
                //console.log(bt);
            ...
            //console.log(el);
            //iframe[i].contents().find(".ytp-chrome-top-buttons").css("display", "none");
            //var bt = el.contents().find(".ytp-chrome-top-buttons");
            //console.log(bt);
            //bt.css("display", "none");
        }
...
odoo.define('website_slides.fullscreen', function (require) {
    ...
    /**
     * This widget is responsible of display Clappr Player
     *
     ...
     */
     ...
     var ClapprPlayer = publicWidget.Widget.extend({
         init: function (parent, slide) {
                this.slide = slide;
                //return this._super.apply(this, arguments);
                this.player = this._setupPlayer(this);
                return this.player;
            },
         /*start: function (){
            var self = this;
         ...
         _setupPlayer: function(){
                var player = new Clappr.Player({
                          source:   this.slide.embedUrl,
                          parentId: "#clappr_" + this.slide.id,
                          width: '100%',
                          playback: {
                          ...
                          autoPlay: true,
                          //events: {
                          //  onEnded: this._ClapprEnded.bind(this)
                          //}
    ...
    var Fullscreen = publicWidget.Widget.extend({
        ...
        */
        ...
        _preprocessSlideData: function (slidesDataList) {
            slidesDataList.forEach(function (slideData, index) {
                ...
                if (slideData.type === 'video') {
					...
					//slideData.embedCode = '<div class="youtubehidecontrols"></div>' + slideData.embedCode;
					//console.log(slideData.embedCode);
                ...
                } else if (slideData.type === 'externalvideo') {
                    slideData.embedCode = slideData.embedCode;
                    slideData.src = $(slideData.embedCode).attr('src');  // embedCode containts an iframe tag, where src attribute is the url (youtube or embed document from odoo)
                    slideData.embedUrl =  'http:' + slideData.src;
                } else if (slideData.type === 'googledrivevideo') {
                    slideData.embedCode = slideData.embedCode;
                    slideData.src = $(slideData.embedCode).attr('src');  // embedCode containts an iframe tag, where src attribute is the url (youtube or embed document from odoo)
                    slideData.embedUrl =  'http:' + slideData.src;
                } else if (slideData.type === 'zoom_meeting') {
                    slideData.embedCode = slideData.embedCode;
                    slideData.src = $(slideData.embedCode).attr('src');  // embedCode containts an iframe tag, where src attribute is the url (youtube or embed document from odoo)
                    slideData.embedUrl =  'http:' + slideData.src;
                } else if (slideData.type == 'clapprvideo'){
                    slideData.src = slideData.embedCode;  // embedCode containts an iframe tag, where src attribute is the url (youtube or embed document from odoo)
                    slideData.embedUrl =  slideData.src;
                    slideData.embedCode = '<div id="clappr_' + slideData.id + '" class="clapprVideo"></div>';
                } else if (slideData.type === 'vimeovideo') {
                    slideData.embedCode = slideData.embedCode;
                    slideData.src = $(slideData.embedCode).attr('src');  // embedCode containts an iframe tag, where src attribute is the url (youtube or embed document from odoo)
                    slideData.embedUrl =  'http:' + slideData.src;
                } else if (slideData.type === 'localvideo') {
                    slideData.embedCode = slideData.embedCode;
                    slideData.src = $(slideData.embedCode).attr('src');  // embedCode containts an iframe tag, where src attribute is the url (youtube or embed document from odoo)
                    slideData.embedUrl =  'http:' + slideData.src;
                }
                ...
                slideData._autoSetDone = _.contains(['infographic', 'presentation', 'document', 'webpage', 'googledrivevideo', 'zoom_meeting'], slideData.type) && !slideData.hasQuestion;
            });
        ...
        _renderSlide: function () {
            ...
            if (_.contains(['document', 'presentation', 'infographic'], slide.type)) {
            ...
            } else if (slide.type === 'externalvideo') {
                ...
                return this.videoPlayer.appendTo($content);
            } else if (slide.type === 'googledrivevideo') {
                this.videoPlayer = $(slide.embedCode);
                /*var gdPlayer = new YT.Player(this.videoPlayer[0], {
                        ...
                        events: {
                            'onReady': gdPlayerReady,
                            'onStateChange': gdPlayerStateChange
                        }
                    });
                console.log('Player:');
                console.log(gdPlayer);
                var gdPlayerReady = function(event){
                    console.log('Player ready');
                    console.log(event);
                    event.target.playVideo();
                };
                var gdPlayerStateChange = function(event){
                ...
                return this.videoPlayer.appendTo($content);
            } else if (slide.type === 'zoom_meeting') {
                this.videoPlayer = $(slide.embedCode);
                return this.videoPlayer.appendTo($content);
            } else if (slide.type === 'clapprvideo') {
                slide.external_url = slide.embedCode;
                this.player = new ClapprPlayer(this, slide);
                this.videoPlayer = $(slide.embedCode);
                $content.append(this.videoPlayer);
                this.player.attachTo(this.videoPlayer[0]);
                //set ended event
                if(!slide.hasQuestion){
                    this.player.on(Clappr.Events.PLAYER_ENDED, function(){
                        slide.data = {};
                        slide.data.id = slide.id;
                        self._onSlideToComplete(slide);
                        this.player = undefined;
                ...
                /*this.videoPlayer = new ClapprPlayer(this, slide);
                return this.videoPlayer.appendTo($content);*/
            } else if (slide.type === 'vimeovideo') {
                this.videoPlayer = $(slide.embedCode);
